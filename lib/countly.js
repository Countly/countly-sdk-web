...
(function (global, factory) {
    ...
    */
    ...
    var CountlyClass = /*#__PURE__*/_createClass(function CountlyClass(ob) {
        ...
        _classPrivateFieldInitSpec(this, _showWidgetInternal, void 0);
        var self = this;
        ...
        this.initialize = function () {
            ...
            _this.hcErrorCount = getValueFromStorage(healthCheckCounterEnum.errorCount) || 0;
            _this.hcWarningCount = getValueFromStorage(healthCheckCounterEnum.warningCount) || 0;
            _this.hcStatusCode = getValueFromStorage(healthCheckCounterEnum.statusCode) || -1;
            _this.hcErrorMessage = getValueFromStorage(healthCheckCounterEnum.errorMessage) || "";
            if (maxCrashLogs && !_this.maxBreadcrumbCount) {
                _this.maxBreadcrumbCount = maxCrashLogs;
                log(logLevelEnums.WARNING, "initialize, 'maxCrashLogs' is deprecated. Use 'maxBreadcrumbCount' instead!");
            } else if (!maxCrashLogs && !_this.maxBreadcrumbCount) {
            ...
            if (_this.clearStoredId) {
                ...
                if (getValueFromStorage("cly_id") && !tempIdModeWasEnabled) {
                    _this.device_id = getValueFromStorage("cly_id");
            ...
            if (isBrowser) {
                if (window.name && window.name.indexOf("cly:") === 0) {
                    ...
                    try {
                        _this.passed_data = JSON.parse(location.hash.replace("#cly:", ""));
                    } catch (ex) {
                        log(logLevelEnums.ERROR, "initialize, Could not parse hash: " + location.hash + ", error: " + ex);
                    }
                }
            }
            if (_this.passed_data && _this.passed_data.app_key && _this.passed_data.app_key === _this.app_key || _this.passed_data && !_this.passed_data.app_key && global) {
                if (_this.passed_data.token && _this.passed_data.purpose) {
                    if (_this.passed_data.token !== getValueFromStorage("cly_old_token")) {
                        setToken(_this.passed_data.token);
                        setValueInStorage("cly_old_token", _this.passed_data.token);
                    }
                    var strippedList = [];
            ...
            if (_this.ignore_visitor) {
                log(logLevelEnums.WARNING, "initialize, ignore_visitor:[" + _this.ignore_visitor + "], this user will not be tracked");
                return;
            }
            ...
            log(logLevelEnums.DEBUG, "initialize, app_key:[" + _this.app_key + "], url:[" + _this.url + "]");
            log(logLevelEnums.DEBUG, "initialize, device_id:[" + getConfig("device_id", ob, undefined) + "]");
            log(logLevelEnums.DEBUG, "initialize, require_consent is enabled:[" + _this.require_consent + "]");
            try {
                log(logLevelEnums.DEBUG, "initialize, metric override:[" + JSON.stringify(_this.metrics) + "]");
                log(logLevelEnums.DEBUG, "initialize, header override:[" + JSON.stringify(_this.headers) + "]");
                ...
                log(logLevelEnums.DEBUG, "initialize, utm tags:[" + JSON.stringify(_this.utm) + "]");
                // empty array printed if non provided
                if (ignoreReferrers) {
                    log(logLevelEnums.DEBUG, "initialize, referrers to ignore :[" + JSON.stringify(ignoreReferrers) + "]");
                }
                log(logLevelEnums.DEBUG, "initialize, salt given:[" + !!_this.salt + "]");
            } catch (e) {
                log(logLevelEnums.ERROR, "initialize, Could not stringify some config object values");
            }
            log(logLevelEnums.DEBUG, "initialize, app_version:[" + _this.app_version + "]");

            // location info printed here
            log(logLevelEnums.DEBUG, "initialize, provided location info; country_code:[" + _this.country_code + "], city:[" + _this.city + "], ip_address:[" + _this.ip_address + "]");

            // print non vital values only if provided by the developer or differs from the default value
            if (_this.namespace !== "") {
                log(logLevelEnums.DEBUG, "initialize, namespace given:[" + _this.namespace + "]");
            }
            if (_this.clearStoredId) {
                log(logLevelEnums.DEBUG, "initialize, clearStoredId flag set to:[" + _this.clearStoredId + "]");
            }
            if (_this.ignore_prefetch) {
                log(logLevelEnums.DEBUG, "initialize, ignoring pre-fetching and pre-rendering from counting as real website visits :[" + _this.ignore_prefetch + "]");
            }
            // if test mode is enabled warn the user
            if (_this.test_mode) {
                log(logLevelEnums.WARNING, "initialize, test_mode:[" + _this.test_mode + "], request queue won't be processed");
            }
            if (_this.test_mode_eq) {
                log(logLevelEnums.WARNING, "initialize, test_mode_eq:[" + _this.test_mode_eq + "], event queue won't be processed");
            }
            // if test mode is enabled warn the user
            if (_this.heatmapWhitelist) {
                log(logLevelEnums.DEBUG, "initialize, heatmap whitelist:[" + JSON.stringify(_this.heatmapWhitelist) + "], these domains will be whitelisted");
            }
            // if storage is se to something other than local storage
            if (_this.storage !== "default") {
                log(logLevelEnums.DEBUG, "initialize, storage is set to:[" + _this.storage + "]");
            }
            if (_this.ignore_bots) {
                log(logLevelEnums.DEBUG, "initialize, ignore traffic from bots :[" + _this.ignore_bots + "]");
            }
            if (_this.force_post) {
                log(logLevelEnums.DEBUG, "initialize, forced post method for all requests:[" + _this.force_post + "]");
            }
            if (_this.remote_config) {
                log(logLevelEnums.DEBUG, "initialize, remote_config callback provided:[" + !!_this.remote_config + "]");
            }
            if (typeof _this.rcAutoOptinAb === "boolean") {
                log(logLevelEnums.DEBUG, "initialize, automatic RC optin is enabled:[" + _this.rcAutoOptinAb + "]");
            }
            ...
            if (_this.enableOrientationTracking) {
                log(logLevelEnums.DEBUG, "initialize, enableOrientationTracking:[" + _this.enableOrientationTracking + "]");
            }
            if (!useSessionCookie) {
            ...
            log(logLevelEnums.DEBUG, "initialize, 'getViewName' callback override provided:[" + (_this.getViewName !== Countly.getViewName) + "]");
            log(logLevelEnums.DEBUG, "initialize, 'getSearchQuery' callback override provided:[" + (_this.getSearchQuery !== Countly.getSearchQuery) + "]");

            // limits are printed here if they were modified 
            if (_this.maxKeyLength !== configurationDefaultValues.MAX_KEY_LENGTH) {
                log(logLevelEnums.DEBUG, "initialize, maxKeyLength set to:[" + _this.maxKeyLength + "] characters");
            }
            if (_this.maxValueSize !== configurationDefaultValues.MAX_VALUE_SIZE) {
                log(logLevelEnums.DEBUG, "initialize, maxValueSize set to:[" + _this.maxValueSize + "] characters");
            }
            if (_this.maxSegmentationValues !== configurationDefaultValues.MAX_SEGMENTATION_VALUES) {
                log(logLevelEnums.DEBUG, "initialize, maxSegmentationValues set to:[" + _this.maxSegmentationValues + "] key/value pairs");
            }
            if (_this.maxBreadcrumbCount !== configurationDefaultValues.MAX_BREADCRUMB_COUNT) {
                log(logLevelEnums.DEBUG, "initialize, maxBreadcrumbCount for custom logs set to:[" + _this.maxBreadcrumbCount + "] entries");
            }
            if (_this.maxStackTraceLinesPerThread !== configurationDefaultValues.MAX_STACKTRACE_LINES_PER_THREAD) {
                log(logLevelEnums.DEBUG, "initialize, maxStackTraceLinesPerThread set to:[" + _this.maxStackTraceLinesPerThread + "] lines");
            }
            if (_this.maxStackTraceLineLength !== configurationDefaultValues.MAX_STACKTRACE_LINE_LENGTH) {
                log(logLevelEnums.DEBUG, "initialize, maxStackTraceLineLength set to:[" + _this.maxStackTraceLineLength + "] characters");
            }
            ...
            // if all fails generate an ID
                ...
                _this.device_id = getConfig("device_id", ob, getStoredIdOrGenerateId());
                if (ob && Object.keys(ob).length) {
                    if (ob.device_id !== undefined) {
                        deviceIdType = DeviceIdTypeInternalEnums.DEVELOPER_SUPPLIED;
                    }
            ...
            setValueInStorage("cly_id", _this.device_id);
        ...
        * */
         ...
         */
        _classPrivateFieldSet2(_showWidgetInternal, this, function (widgetType, nameIDorTag) {
            log(logLevelEnums.INFO, "showWidget, Showing ".concat(widgetType, " widget, nameIDorTag:[").concat(nameIDorTag, "]"));
            _this.get_available_feedback_widgets(function (feedbackWidgetArray, error) {
                if (error) {
                    log(logLevelEnums.ERROR, "showWidget, Error while getting feedback widgets list: ".concat(error));
                    return;
                }
                ...
                if (nameIDorTag && typeof nameIDorTag === 'string') {
                    ...
                    if (matchedWidget) {
                        ...
                        log(logLevelEnums.VERBOSE, "showWidget, Found ".concat(widgetType, " widget by name, id, or tag: [").concat(JSON.stringify(matchedWidget), "]"));
                    }
                }
                if (!widget) {
                    log(logLevelEnums.ERROR, "showWidget, No ".concat(widgetType, " widget found."));
                    return;
                }
