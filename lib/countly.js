...
(function (Countly) {
    ...
    var inited = false,
        ...
        hasPulse = false,
        shouldConsent = false,
        consents = {sessions:{},events:{},views:{},scrolls:{},clicks:{},forms:{},crashes:{},push:{},attribution:{},users:{}},
        syncConsents = {},
        urlParseRE = /^(((([^:\/#\?]+:)?(?:(\/\/)((?:(([^:@\/#\?]+)(?:\:([^:@\/#\?]+))?)@)?(([^:\/#\?\]\[]+|\[[^\/\]@#?]+\])(?:\:([0-9]+))?))?)?)?((\/?(?:[^\/\?#]+\/+)*)([^\?#]*)))?(\?[^#]+)?)(#.*)?/,
 ...
 * @param {boolean} [requireConsent=false] - Pass true if you are implementing GDPR compatible consent management. It would prevent running any functionality without proper consent
 * @example
 * Countly.init({
 *   //provide your app key that you retrieved from Countly dashboard
 ...
 */
    Countly.init = function(ob, requireConsent){
        if(!inited){
            shouldConsent = requireConsent || false;
    ...
    * @param {object} features - object to define consent name as key and features as value
    * @example <caption>Adding all features under one consent</caption>
    ...
    * //After this call Countly.add_consent("all") to allow all features
    @example <caption>Grouping consents</caption>
    * Countly.group_features({activity:["sessions","events","views"],interaction:["scrolls","clicks","forms"]});
    ...
    Countly.add_consent = function(feature){
        if(consents[feature]){
            if(consents[feature].features){
                ...
                for(var i = 0; consents[feature].features.length; i++){
                    Countly.add_consent(consents[feature].features[i]);
                }
            }
            else{
    ...
    Countly.remove_consent = function(feature){
        if(consents[feature]){
            if(consents[feature].features){
                //this is added group, let's iterate through sub features
                for(var i = 0; consents[feature].features.length; i++){
                    Countly.remove_consent(consents[feature].features[i]);
                }
            }
            else{
                //this is core feature
