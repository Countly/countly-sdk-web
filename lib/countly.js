    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        this.initialize = function() {
            ...
            try {
                // an array to be filled with given consent
                var givenConsents = [];
                // loop through the consents and add the given ones to the array
                for (var val = 0, len = Countly.features.length; val < len; val++) {
                    var cons = consents[val];
                    if (cons) {
                        givenConsents.push(cons);
                    }
                }
                log(logLevelEnums.DEBUG, "initialize, require_consent:[" + this.require_consent + "], and given consents:[" + JSON.stringify(givenConsents) + "]");
                ...
                if (this.onload) {
                    log(logLevelEnums.DEBUG, "initialize, onload:[" + JSON.stringify(this.onload) + "]");
                }
                ...
                if (this.utm !== { source: true, medium: true, campaign: true, term: true, content: true }) {
                    log(logLevelEnums.DEBUG, "initialize, utm:[" + JSON.stringify(this.utm) + "]");
                }
                // empty array printed if non provided
                if (ignoreReferrers) {
                    log(logLevelEnums.DEBUG, "initialize, ignore_referrers:[" + JSON.stringify(ignoreReferrers) + "]");
                }
            }
            catch (e) {
            ...
            log(logLevelEnums.DEBUG, "initialize, location info: [country_code:[" + this.country_code + "], city:[" + this.city + "], ip_address:[" + this.ip_address + "]]");

            // print non vital values only if provided by the developer or differs from the default value
            if (this.namespace !== "") {
                log(logLevelEnums.DEBUG, "initialize, namespace:[" + this.namespace + "]");
            }
            if (this.clearStoredId) {
                log(logLevelEnums.DEBUG, "initialize, clearStoredId:[" + this.clearStoredId + "]");
            }
            if (this.ignore_prefetch) {
                log(logLevelEnums.DEBUG, "initialize, ignore_prefetch:[" + this.ignore_prefetch + "]");
            }
            ...
            if (this.storage !== "default") {
                log(logLevelEnums.DEBUG, "initialize, storage:[" + this.storage + "]");
            }
            if (this.ignore_bots) {
                log(logLevelEnums.DEBUG, "initialize, ignore_bots:[" + this.ignore_bots + "]");
            }
            if (this.force_post) {
                log(logLevelEnums.DEBUG, "initialize, force_post:[" + this.force_post + "]");
            }
            if (this.remote_config) {
                log(logLevelEnums.DEBUG, "initialize, remote_config provided:[" + !!this.remote_config + "]");
            }
            ...
            if (this.track_domains) {
                log(logLevelEnums.DEBUG, "initialize, track_domains:[" + this.track_domains + "]");
            }
            ...
            log(logLevelEnums.DEBUG, "initialize, getViewName provided:[" + !!this.getViewName + "]");
            log(logLevelEnums.DEBUG, "initialize, getSearchQuery provided:[" + !!this.getSearchQuery + "]");

            // limits are printed here if they were modified 
            if (this.maxKeyLength !== 128) {
                log(logLevelEnums.DEBUG, "initialize, maxKeyLength:[" + this.maxKeyLength + "]");
            }
            if (this.maxValueSize !== 256) {
                log(logLevelEnums.DEBUG, "initialize, maxValueSize:[" + this.maxValueSize + "]");
            }
            if (this.maxSegmentationValues !== 30) {
                log(logLevelEnums.DEBUG, "initialize, maxSegmentationValues:[" + this.maxSegmentationValues + "]");
            }
            if (this.maxBreadcrumbCount !== 100) {
                log(logLevelEnums.DEBUG, "initialize, maxBreadcrumbCount:[" + this.maxBreadcrumbCount + "]");
            }
            if (this.maxStackTraceLinesPerThread !== 30) {
                log(logLevelEnums.DEBUG, "initialize, maxStackTraceLinesPerThread:[" + this.maxStackTraceLinesPerThread + "]");
            }
            if (this.maxStackTraceLineLength !== 200) {
                log(logLevelEnums.DEBUG, "initialize, maxStackTraceLineLength:[" + this.maxStackTraceLineLength + "]");
            }
            if (beatInterval !== 500) {
                log(logLevelEnums.DEBUG, "initialize, interval:[" + beatInterval + "]");
            }
            if (queueSize !== 1000) {
                log(logLevelEnums.DEBUG, "initialize, queue_size:[" + queueSize + "]");
            }
            if (failTimeoutAmount !== 60) {
                log(logLevelEnums.DEBUG, "initialize, fail_timeout:[" + failTimeoutAmount + "]");
            }
            if (inactivityTime !== 20) {
                log(logLevelEnums.DEBUG, "initialize, inactivity_time:[" + inactivityTime + "]");
            }
            if (sessionUpdate !== 60) {
                log(logLevelEnums.DEBUG, "initialize, session_update:[" + sessionUpdate + "]");
            }
            if (maxEventBatch !== 100) {
                log(logLevelEnums.DEBUG, "initialize, max_events:[" + maxEventBatch + "]");
            }
            if (maxCrashLogs) {
                log(logLevelEnums.DEBUG, "initialize, max_logs:[" + maxCrashLogs + "]");
            }
            if (sessionCookieTimeout !== 30) {
                log(logLevelEnums.DEBUG, "initialize, session_cookie_timeout:[" + sessionCookieTimeout + "]");
            }
