...
(function (Countly) {
	...
	Countly.init = function(ob){
		if(!inited){
			...
			Countly.ignore_bots = ob.ignore_bots || Countly.ignore_bots || false;
			Countly.q = Countly.q || [];
    ...
    **************************/
    Countly.change_custom_property = function(key, value, mod){
        var prop = {};
        prop[key] = {};
        prop[key]["$"+mod] = value;
        toRequestQueue({user_details: JSON.stringify(prop)});
    };
	...
	**/
    ...
    function recordError(err, nonfatal){
        ...
        ob._custom.Page = window.location;
        try{
