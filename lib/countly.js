...
(function (root, factory) {
    ...
    **/
    Countly.initialize_nps = function(callback) {
        if (offlineMode) {
            log("Cannot enable surveys in offline mode");
        }
        else {
            initialize_surveys("nps", function(err, widgets) {
                if (err) {
                    if (callback) {
    ...
    /**
    * Initialize In app survey
    * @param {Function} callback - Callback function
    **/
    Countly.initialize_survey = function(callback) {
        if (offlineMode) {
            log("Cannot enable surveys in offline mode");
        }
        else {
            initialize_surveys("survey", function(err, widgets) {
                if (err) {
                    if (callback) {
                        return callback(false, err);
                    }
    ...
    **/
    function initialize_surveys(type, cb) {
        ...
        sendXmlHttpRequest(url, data, function(err, params, responseText){
            if(err) {
                log("Error occured while fetching surveys", err);
                return cb(err);
            }

            try {
                var response = JSON.parse(responseText);
                var widgets = response.result;
                return cb(null, widgets);
            } catch (error) {
    ...
    **/
    ...
    function processSurveys(currSurvey) {
        try {
            var wrapper = document.createElement("div");
            wrapper.className = 'countly-surveys-wrapper';
            wrapper.id = 'countly-surveys-wrapper-' + currSurvey._id;

            if (currSurvey.type === "survey") {
                //Set popup position
                wrapper.className = wrapper.className + " " + currSurvey.appearance.position;
            }

            var url = Countly.url;

            if (currSurvey.type === "nps") {
                url += "/feedback/nps";
            }
            else if (currSurvey.type === "survey") {
                url += "/feedback/survey";
            }
            else {
                return;
            ...
            //Only web SDK passes origin and source
            ...
            if (currSurvey.type === "survey") {
                ...
                case "onAbandon":
                    var lastY = 0;
                    add_event(window, "mousemove", function(e) {
                        if (!surveyShown) {
                            lastY = Math.max(lastY, e.y);
                            if (lastY > 100 && e.y < 100) {
                                surveyShown = true;
                                document.getElementById('countly-surveys-wrapper-' + currSurvey._id).style.display = "block";
                                document.getElementById('csbg').style.display = "block";
                            }
