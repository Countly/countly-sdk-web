...
(function (root, factory) {
    if(typeof define === "function" && define.amd) {
        define([], function(){ return factory(root.Countly, root);});
    } else if(typeof module === "object" && module.exports) {
        module.exports = factory(root.Countly, root);
    } else {
        root.Countly = factory(root.Countly, root);
    }
}(typeof window !== "undefined" ? window : this, function (Countly, root) {
    "use strict";
    ...
    /**
     *  Countly class
     *  @param {Obeject} ob - configuration object
     *  @returns {Countly} Countly instance51
     */
    root.CountlyClass = function(ob) {
        var self = this,
            global = !Countly.i,
        ...
        var consents = {}, i = 0;
        for (i = 0; i < Countly.features.length; i++) {
        ...
        **/
        ...
        function heartBeat() {
            ...
            if (global && typeof Countly.q !== "undefined" && Countly.q.length > 0) {
                ...
                for (i = 0; i < q.length; i++) {
                    ...
                    if (typeof req === "function") {
                        ...
                        if (typeof inst[req[arg]] !== "undefined") {
                            inst[req[arg]].apply(inst, req.slice(1));
                        }
                        else if (req[arg].indexOf("userData.") === 0) {
                            var userdata = req[arg].replace("userData.", "");
                            if (typeof inst.userData[userdata] !== "undefined") {
                                inst.userData[userdata].apply(inst, req.slice(1));
                            }
                        }
                    }
                }
            ...
            if (!offlineMode && requestQueue.length > 0 && readyToProcess && getTimestamp() > failTimeout) {
                ...
                store("cly_queue", requestQueue, true);
                sendXmlHttpRequest(self.url + apiPath, params, function(err, params){
        ...
        function store(key, value, storageOnly) {
            //apply namespace
    ...
    Countly.init = function (ob) {
        ob = ob || {};
        var appKey = ob.app_key || Countly.app_key;
        if (!Countly.i || !Countly.i[appKey]) {
            var inst = new root.CountlyClass(ob);
            if (!Countly.i) {
                Countly.i = {};
                for (var key in Countly) {
                    if (!inst[key]) {
                        inst[key] = Countly[key];
                    }
                }
                root.Countly = inst;
            }
