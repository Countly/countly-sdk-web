...
(function(root, factory) {
    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        var feedbackWidgetSegmentation = null; // this is the global variable for tracking the segmentation of the feedback widget. Used in feedback widget.
        ...
        * */
        this.present_feedback_widget = function(presentableFeedback, id, className) {
            ...
            try {
                ...
                if (feedbackWidgetSegmentation) {
                    url += "&segmentation=" + JSON.stringify(feedbackWidgetSegmentation);
                }
        ...
        /**
         * Add any key value pairs to be send with the feedback widgets
         * @param {Object} segmentation - segmentation object with key value pairs to add custom segmentation to feedback widgets
         */
        this.addFeedbackWidgetSegmentation = function(segmentation) {
            log(logLevelEnums.INFO, "[addFeedbackWidgetSegmentation] Adding segmentation to feedback widgets:[" + JSON.stringify(segmentation) + "]");
            if (!segmentation || typeof segmentation !== "object") {
                log(logLevelEnums.ERROR, "[addFeedbackWidgetSegmentation] Segmentation is not an object");
                return;
            }
            if (Object.keys(segmentation).length === 0) {
                log(logLevelEnums.ERROR, "[addFeedbackWidgetSegmentation] Segmentation is empty");
                return;
            }
            feedbackWidgetSegmentation = segmentation;
        };
