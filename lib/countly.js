        ...
        function sendXmlHttpRequest(url, params, callback) {
            try {
                ...
                xhr.onreadystatechange = function() {
                    //check if operations done, status code is acceptable and the response is JSON 
                    if (this.readyState === 4 && this.status >= 200 && this.status < 300 && this.responseType === "json") {
                        //check is callback is a function and response includes a result field
                        if (typeof callback === 'function' && JSON.stringify(this.response).includes("result")) {
                            callback(false, params, JSON.stringify(this.response));
                        }
                    }
