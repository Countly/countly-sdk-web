    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        this.initialize = function() {
            ...
            // assign current configuration values to that object
            try {
                // turning functions into string so that we can print them in console 
                configurationObject.serialize = this.serialize.toString() || null;
                configurationObject.deserialize = this.deserialize.toString() || null;
                configurationObject.getViewName = this.getViewName.toString() || null;
                configurationObject.getSearchQuery = this.getSearchQuery.toString() || null;
            }
            catch (error) {
                log(logLevelEnums.ERROR, "initialize, Could not convert configuration functions into string:" + error);
            }
            configurationObject.DeviceIdType = this.DeviceIdType || null;
            configurationObject.namespace = this.namespace || null;
            configurationObject.clearStoredId = this.clearStoredId || null;
            configurationObject.app_key = this.app_key || null;
            configurationObject.onload = this.onload || null;
            configurationObject.utm = this.utm || null;
            configurationObject.ignore_prefetch = this.ignore_prefetch || null;
            configurationObject.debug = this.debug || null;
            configurationObject.test_mode = this.test_mode || null;
            configurationObject.metrics = this.metrics || null;
            configurationObject.headers = this.headers || null;
            configurationObject.url = this.url || null;
            configurationObject.app_version = this.app_version || null;
            configurationObject.country_code = this.country_code || null;
            configurationObject.city = this.city || null;
            configurationObject.ip_address = this.ip_address || null;
            configurationObject.ignore_bots = this.ignore_bots || null;
            configurationObject.force_post = this.force_post || null;
            configurationObject.remote_config = this.remote_config || null;
            configurationObject.ignore_visitor = this.ignore_visitor || null;
            configurationObject.require_consent = this.require_consent || null;
            configurationObject.track_domains = this.track_domains || null;
            configurationObject.storage = this.storage || null;
            configurationObject.enableOrientationTracking = this.enableOrientationTracking || null;
            configurationObject.maxKeyLength = this.maxKeyLength || null;
            configurationObject.maxValueSize = this.maxValueSize || null;
            configurationObject.maxSegmentationValues = this.maxSegmentationValues || null;
            configurationObject.maxBreadcrumbCount = this.maxBreadcrumbCount || null;
            configurationObject.maxStackTraceLinesPerThread = this.maxStackTraceLinesPerThread || null;
            configurationObject.serializationFunction = this.maxStackTraceLinesPerThread || null;
            try {
                // stringify the object to display in console
                configurationObject = JSON.stringify(configurationObject);
            }
            catch (error) {
                log(logLevelEnums.ERROR, "initialize, Could not stringify the configuration object:" + error);
            }
            log(logLevelEnums.INFO, "initialize, Countly initialized with the following configuration: [" + configurationObject + "]");
