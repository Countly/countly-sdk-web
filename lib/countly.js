        ...
        this.initialize = function() {
            ...
            if (window.name && window.name.indexOf("cly:") === 0) {
                try {
                    ...
                    log(logLevelEnums.ERROR, "initialize, Could not parse name: " + ex, window.name);
                }
            }
                ...
                try {
                    ...
                    log(logLevelEnums.ERROR, "initialize, Could not parse hash: " + ex, location.hash);
                }
            }
        ...
        * */
        ...
        function add_cly_events(event) {
            ...
            if (self.ignore_visitor) {
                log(logLevelEnums.ERROR, "Adding event failed. Ignoring bots");
                return;
            }
            ...
            if (ignoreList && ignoreList.length) {
                for (var i = 0; i < ignoreList.length; i++) {
                    try {
                        ...
                        log(logLevelEnums.ERROR, "track_pageview, Problem with regex: " + ignoreList[i] + ", error: " + ex);
                    }
                }
            }
            lastView = page;
        ...
        * */
        this.present_feedback_widget = function(presentableFeedback, id, className) {
            ...
            try {
                ...
                // wrapper.id = "countly-iframe-wrapper-" + presentableFeedback._id;
                ...
                log(logLevelEnums.DEBUG, "present_feedback_widget, appended the iframe to wrapper");

                add_event(window, "message", function(e) {
                    var data = {};
                    try {
                        data = JSON.parse(e.data);
                        log(logLevelEnums.INFO, "present_feedback_widget, Parsed response message " + data);
                    }
                    catch (ex) {
                ...
                log(logLevelEnums.ERROR, "present_feedback_widget, Something went wrong while presenting the feedback: " + e);
            }
                    ...
                    if (ignoreReferrers && ignoreReferrers.length) {
                        for (var k = 0; k < ignoreReferrers.length; k++) {
                            try {
                                ...
                                log(logLevelEnums.ERROR, "Problem with regex: " + ignoreReferrers[k], ", error: " + ex);
                            }
                        }
                    }
                    if (!ignoring) {
         ...
         *  @param {Boolean} expectBroadResponse - if true that means the expected response is either a JSON object or a JSON array, if false only JSON 
         */
        function sendXmlHttpRequest(functionName, url, params, callback, expectBroadResponse) {
            expectBroadResponse = expectBroadResponse || false;
            try {
                ...
                xhr.onreadystatechange = function() {
                    if (this.readyState === 4) {
                        ...
                        // response validation function will be selected to accept JSON arrays if expectBroadResponse is true
                        var isResponseValidated;
                        if (expectBroadResponse) {
                            // JSON array/object both can pass
                            isResponseValidated = isResponseValidBroad(this.status, this.responseText, expectBroadResponse);
                        }
                        else {
                            // only JSON object can pass
                            isResponseValidated = isResponseValid(this.status, this.responseText, expectBroadResponse);
                        }
