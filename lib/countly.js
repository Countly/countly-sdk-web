                ...
                sendXmlHttpRequest(this.url + '/o/feedback/widget', {widget_id: id}, function(err, params, responseText) {
                    if (err) {
                       log(logLevelEnums.ERROR, "presentRatingWidgetWithID, Error occurred", err);
                        ...
                        // widget object
                        var currentWidget = responseText;
               ...
               log(logLevelEnums.DEBUG, "Sending XML HTTP request");
                ...
                xhr.onreadystatechange = function() {
                    if (this.readyState === 4 && this.status >= 200 && this.status < 300) {
                        if (typeof callback === 'function' && (this.responseText.indexOf("result") >= 0)) {
                            try {
                                var parsedResponse = JSON.parse(this.responseText);
                                callback(false, params, parsedResponse);
                            }
                            catch (e) {
                               log(logLevelEnums.ERROR, "XML HTTP response was in wrong format", e);
                            }
                        }
                    }
