    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        this.initialize = function() {
            ...
            log(logLevelEnums.INFO, "initialize, app_key:[" + this.app_key + "], url:[" + this.url + "]");
            log(logLevelEnums.INFO, "initialize, device_id:[" + getConfig("device_id", ob, undefined) + "]");
            try {
                log(logLevelEnums.INFO, "initialize, require_consent:[" + this.require_consent + "], and consent state:[" + JSON.stringify(consents) + "]");
                log(logLevelEnums.INFO, "initialize, metric override:[" + JSON.stringify(this.metrics) + "]");
                log(logLevelEnums.INFO, "initialize, header override:[" + JSON.stringify(this.headers) + "]");
                // empty array is truthy and so would be printed if provided
                if (this.onload) {
                    log(logLevelEnums.INFO, "initialize, onload:[" + JSON.stringify(this.onload) + "]");
                }
                // if the utm object is different to default utm object print it here
                if (this.utm !== { source: true, medium: true, campaign: true, term: true, content: true }) {
                    log(logLevelEnums.INFO, "initialize, utm:[" + JSON.stringify(this.utm) + "]");
                }
                // empty array printed if non provided
                if (ignoreReferrers) {
                    log(logLevelEnums.INFO, "initialize, ignore_referrers:[" + JSON.stringify(ignoreReferrers) + "]");
                }
            }
            catch (e) {
                log(logLevelEnums.ERROR, "initialize, Could not stringify some config object values");
            }
            log(logLevelEnums.INFO, "initialize, app_version:[" + this.app_version + "]");

            // location info printed here
            log(logLevelEnums.INFO, "initialize, location info: [country_code:[" + this.country_code + "], city:[" + this.city + "], ip_address:[" + this.ip_address + "]]");

            // print non vital values only if provided by the developer or differs from the default value
            if (this.namespace !== "") {
                log(logLevelEnums.INFO, "initialize, namespace:[" + this.namespace + "]");
            }
            if (this.clearStoredId) {
                log(logLevelEnums.INFO, "initialize, clearStoredId:[" + this.clearStoredId + "]");
            }
            if (this.ignore_prefetch) {
                log(logLevelEnums.INFO, "initialize, ignore_prefetch:[" + this.ignore_prefetch + "]");
            }
            // if test mode is enabled warn the user
            if (this.test_mode) {
                log(logLevelEnums.WARNING, "initialize, test_mode:[" + this.ignore_prefetch + "], queues won't be processed");
            }
            // if storage is se to something other than local storage
            if (this.storage !== "default") {
                log(logLevelEnums.INFO, "initialize, storage:[" + this.storage + "]");
            }
            if (this.ignore_bots) {
                log(logLevelEnums.INFO, "initialize, ignore_bots:[" + this.ignore_bots + "]");
            }
            if (this.force_post) {
                log(logLevelEnums.INFO, "initialize, force_post:[" + this.force_post + "]");
            }
            if (this.remote_config) {
                log(logLevelEnums.INFO, "initialize, remote_config provided:[" + !!this.remote_config + "]");
            }
            ...
            if (this.track_domains) {
                log(logLevelEnums.INFO, "initialize, track_domains:[" + this.track_domains + "]");
            }
            if (this.enableOrientationTracking) {
                log(logLevelEnums.INFO, "initialize, enableOrientationTracking:[" + this.enableOrientationTracking + "]");
            }
            if (!useSessionCookie) {
                log(logLevelEnums.WARNING, "initialize, use_session_cookie disabled:[" + useSessionCookie + "]");
            }
            if (offlineMode) {
                log(logLevelEnums.INFO, "initialize, offline_mode:[" + offlineMode + "], user info won't be send to the servers");
            }
            // functions, if provided, would be printed as true without revealing their content
            log(logLevelEnums.INFO, "initialize, getViewName provided:[" + !!this.getViewName + "]");
            log(logLevelEnums.INFO, "initialize, getSearchQuery provided:[" + !!this.getSearchQuery + "]");

            // limits are printed here if they were modified 
            if (this.maxKeyLength !== 128) {
                log(logLevelEnums.INFO, "initialize, maxKeyLength:[" + this.maxKeyLength + "]");
            }
            if (this.maxValueSize !== 256) {
                log(logLevelEnums.INFO, "initialize, maxValueSize:[" + this.maxValueSize + "]");
            }
            if (this.maxSegmentationValues !== 30) {
                log(logLevelEnums.INFO, "initialize, maxSegmentationValues:[" + this.maxSegmentationValues + "]");
            }
            if (this.maxBreadcrumbCount !== 100) {
                log(logLevelEnums.INFO, "initialize, maxBreadcrumbCount:[" + this.maxBreadcrumbCount + "]");
            }
            if (this.maxStackTraceLinesPerThread !== 30) {
                log(logLevelEnums.INFO, "initialize, maxStackTraceLinesPerThread:[" + this.maxStackTraceLinesPerThread + "]");
            }
            if (this.maxStackTraceLineLength !== 200) {
                log(logLevelEnums.INFO, "initialize, maxStackTraceLineLength:[" + this.maxStackTraceLineLength + "]");
            }
            if (beatInterval !== 500) {
                log(logLevelEnums.INFO, "initialize, interval:[" + beatInterval + "]");
            }
            if (queueSize !== 1000) {
                log(logLevelEnums.INFO, "initialize, queue_size:[" + queueSize + "]");
            }
            if (failTimeoutAmount !== 60) {
                log(logLevelEnums.INFO, "initialize, fail_timeout:[" + failTimeoutAmount + "]");
            }
            if (inactivityTime !== 20) {
                log(logLevelEnums.INFO, "initialize, inactivity_time:[" + inactivityTime + "]");
            }
            if (sessionUpdate !== 60) {
                log(logLevelEnums.INFO, "initialize, session_update:[" + sessionUpdate + "]");
            }
            if (maxEventBatch !== 100) {
                log(logLevelEnums.INFO, "initialize, max_events:[" + maxEventBatch + "]");
            }
            if (maxCrashLogs) {
                log(logLevelEnums.INFO, "initialize, max_logs:[" + maxCrashLogs + "]");
            }
            if (sessionCookieTimeout !== 30) {
                log(logLevelEnums.INFO, "initialize, session_cookie_timeout:[" + sessionCookieTimeout + "]");
            }
