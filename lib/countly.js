...
(function(root, factory) {
    ...
    var CountlyClass = function(ob) {
        ...
        * */
        this.report_feedback = function(ratingWidget) {
            log(logLevelEnums.WARNING, "report_feedback, Deprecated function call! Use 'recordRatingWidgetWithID' or 'recordFeedbackWidgetWithID' in place of this call. Call will be redirected to 'recordRatingWidgetWithID' now!");
        ...
        * */
        ...
        /**
        * Provide information about survey or nps
        * @param {String} type - widget type, 'nps' or 'survey'
        * @param {Object} widget - object with rating widget properties
        * Possible keys for nps
        * @param {string} widget.widget_id - id of the widget in the dashboard
        * @param {string=} widget.platform - user's platform (will be filled if not provided)
        * @param {string=} widget.app_version - app's app version (will be filled if not provided)
        * @param {number} widget.rating - user's rating from 0-10
        * @param {number} widget.comment - user's comment
        * Survey would need answers
        * @param {Array} widget.answers - user's answers in an array of objects
        * @param {Object} widget.answers[0] - answer object: {answer:'Answer'||(or number like 5), id:'ch123456'}
        * @param {String|Number} widget.answers[0].answer - answer text (string), rating (number) or multi choice answer id (sting. If more than one choice, separated bu comma). 
        * @param {String} widget.answers[0].id - question id
        * 
        * */
        this.recordFeedbackWidgetWithID = function(type, widget) {
            log(logLevelEnums.INFO, "recordFeedbackWidgetWithID, Providing information about user from: [" + type + "] with ID: [ " + widget.widget_id + " ]");

            // checks for vital information
            if (!this.check_consent(featureEnums.FEEDBACK)) {
                return;
            }
            if (type !== "nps" && type !== "survey") {
                log(logLevelEnums.ERROR, "recordFeedbackWidgetWithID, Entered type [" + type + "] must be 'survey' or 'nps'");
                return;
            }
            if (!widget.widget_id) {
                log(logLevelEnums.ERROR, "recordFeedbackWidgetWithID, Feedback Widgets must contain widget_id property");
                return;
            }

            // type specific checks to see if everything was provided
            var props = ["widget_id", "platform", "app_version"];
            if (type === "nps") {
                if (!widget.rating) {
                    log(logLevelEnums.ERROR, "recordFeedbackWidgetWithID, Widget must contain rating property");
                    return;
                }
                props.push("rating", "comment");
            }
            else if (type === "survey") {
                if (widget.answers.length < 1 || !answerValidator(widget.answers)) {
                    log(logLevelEnums.ERROR, "recordFeedbackWidgetWithID, Widget should have proper answers to be reported");
                    return;
                }
            }
            ...
            // event template
            var event = {
                key: type === "nps" ? internalEventKeyEnums.NPS : internalEventKeyEnums.SURVEY,
                count: 1,
                segmentation: {}
            };

            // prepare segmentation
            event.segmentation = getProperties(widget, props);

            // reformatting the answers so that server can recognize them in case of survey
            if (type === "survey") {
                for (var i = 0, len = widget.answers.length; i < len; i++) {
                    if (widget.answers[i].id && widget.answers[i].answer) {
                        var key = "answ-" + widget.answers[i].id;
                        var val = widget.answers[i].answer;
                        event.segmentation[key] = val;
                    }
                }
            }
            else {
                if (event.segmentation.rating > 10) {
                    log(logLevelEnums.WARNING, "recordFeedbackWidgetWithID, You have entered a rating higher than 10. Changing it back to 10 now.");
                    event.segmentation.rating = 10;
                }
                else if (event.segmentation.rating < 0) {
                    log(logLevelEnums.WARNING, "recordFeedbackWidgetWithID, You have entered a rating lower than 0. Changing it back to 0 now.");
                    event.segmentation.rating = 0;
                }
            }
            // app version can be obtained from metrics
            if (!event.segmentation.app_version) {
                event.segmentation.app_version = this.metrics._app_version || this.app_version;
            }

            // add event
            log(logLevelEnums.INFO, "recordFeedbackWidgetWithID, Reporting " + type + ": ", event);
            add_cly_events(event);
        };
        /**
        ...
        * */
        ...
        /**
        * Get feedback (nps and survey) widget data, like questions, message etc.
        * @param {Array} widgetArray - Array of widget objects, probably retrieved from 'get_available_feedback_widgets'
        * @param {Function} callback - Callback function with two parameters, 1st for returned widget data, 2nd for error
        * */
        this.getFeedbackWidgetData = function(widgetArray, callback) {
            if (!Array.isArray(widgetArray) && widgetArray.length < 1) {
                log(logLevelEnums.ERROR, "getFeedbackWidgetData, Expected to get an array of widget objects but got: [" + widgetArray + "], aborting.");
                return;
            }
            log(logLevelEnums.INFO, "getFeedbackWidgetData, Retrieving data for: [" + JSON.stringify(widgetArray) + "], callback function is provided:[" + (!!callback) + "]");
            if (!this.check_consent(featureEnums.FEEDBACK)) {
                if (callback) {
                    callback(null, new Error("Consent for feedback not provided."));
                }
            ...
            // incase rating widget info is included or multiple widgets are present we should sort out the list and group ids together before contacting the server
            var npsId = "";
            var surveyId = "";
            for (var i = 0, l = widgetArray.length; i < l; i++) {
                if (widgetArray[i].type === "nps") {
                    if (npsId) {
                        npsId += ("," + widgetArray[i]._id);
                    }
                    else {
                        // first id without comma
                        npsId += widgetArray[i]._id;
                    }
                }
                else if (widgetArray[i].type === "survey") {
                    if (surveyId) {
                        surveyId += ("," + widgetArray[i]._id);
                    }
                    else {
                        // first id without comma
                        surveyId += widgetArray[i]._id;
            ...
            if (!npsId && !surveyId) {
                log(logLevelEnums.ERROR, "getFeedbackWidgetData, None of the widgets are nps or survey. Aborting.");
                return;
            }
            ...
            var data;
            if (npsId) {
                url = this.url + "/o/surveys/nps/widget";
                data = {
                    widget_ids: npsId,
                };
            ...
            if (surveyId) {
                url = this.url + "/o/surveys/survey/widget";
                data = {
                    widget_ids: surveyId,
                };
                sendXmlHttpRequest("getFeedbackWidgetData", url, data, responseCallback, true);
    ...
    * */
    ...
    /** ~
     *  Checks if the provided answers array is valid
     *  @memberof Countly._internals
     *  @param {Array} answers - array with survey answers
     *  @returns {Boolean} returns true if the array is valid
     */
    function answerValidator(answers) {
        var errorCount = 0;
        for (var i = 0, len = answers.length; i < len; i++) {
            if (typeof answers[i].id !== "string" && !answers[i].id) {
                errorCount++;
            }
            else if (typeof answers[i].answer !== "string" && typeof answers[i].answer !== "number") {
                errorCount++;
            }
        }
        // if at least 1 answer is correct pass valid
        return errorCount < answers.length;
    }
