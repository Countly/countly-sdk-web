...
(function (root, factory) {
    ...
    Countly.init = function (ob, requireConsent) {
        if (!inited) {
            ...
            if (!Countly.ignore_visitor) {
                ...
                // if test mode enabled; define sample values
        ...
        if (Countly.device_id !== newId) {
            if (!merge) {
                ...
                if (eventQueue.length > 0) {
                    ...
                    // if test mode enabled; don't make empty eventQueue
                    if (!Countly.test_mode) {
                        eventQueue = [];
    ...
    **/
    ...
    function toRequestQueue(request) {
        ...
        // if test mode enabled; push request into exposed requestQueue array too
        if (Countly.test_mode) {
            Countly._internals.requestQueue.push(request);
        }
    ...
    function heartBeat() {
        ...
        if (eventQueue.length > 0) {
            if (eventQueue.length <= maxEventBatch) {
                toRequestQueue({ events: JSON.stringify(eventQueue) });
                // if test_mode enabled; don't make empty eventQueue 
                if (!Countly.test_mode) eventQueue = [];
            }
            else {
        ...
        //stop process requestQueue when test_mode enabled
        if (!Countly.test_mode && (requestQueue.length > 0 && readyToProcess && getTimestamp() > failTimeout)) {
