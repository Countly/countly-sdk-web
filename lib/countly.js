...
(function(root, factory) {
     ...
     */
    var numericalConfigurationDefaultValues = {
        BEAT_INTERVAL: 500,
        QUEUE_SIZE: 1000,
    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        var apiPath = "/i";
        var readPath = "/o/sdk";
        var beatInterval = getConfig("interval", ob, numericalConfigurationDefaultValues.BEAT_INTERVAL);
        var queueSize = getConfig("queue_size", ob, numericalConfigurationDefaultValues.QUEUE_SIZE);
        var requestQueue = [];
        var eventQueue = [];
        var remoteConfigs = {};
        var crashLogs = [];
        ...
        var failTimeout = 0;
        var failTimeoutAmount = getConfig("fail_timeout", ob, numericalConfigurationDefaultValues.FAIL_TIMEOUT_AMOUNT);
        var inactivityTime = getConfig("inactivity_time", ob, numericalConfigurationDefaultValues.INACTIVITY_TIME);
        var inactivityCounter = 0;
        var sessionUpdate = getConfig("session_update", ob, numericalConfigurationDefaultValues.SESSION_UPDATE);
        var maxEventBatch = getConfig("max_events", ob, numericalConfigurationDefaultValues.MAX_EVENT_BATCH);
        var maxCrashLogs = getConfig("max_logs", ob, null);
        var useSessionCookie = getConfig("use_session_cookie", ob, true);
        var sessionCookieTimeout = getConfig("session_cookie_timeout", ob, numericalConfigurationDefaultValues.SESSION_COOKIE_TIMEOUT);
        ...
        this.initialize = function() {
            ...
            this.maxKeyLength = getConfig("max_key_length", ob, numericalConfigurationDefaultValues.MAX_KEY_LENGTH);
            this.maxValueSize = getConfig("max_value_size", ob, numericalConfigurationDefaultValues.MAX_VALUE_SIZE);
            this.maxSegmentationValues = getConfig("max_segmentation_values", ob, numericalConfigurationDefaultValues.MAX_SEGMENTATION_VALUES);
            this.maxBreadcrumbCount = getConfig("max_breadcrumb_count", ob, null);
            this.maxStackTraceLinesPerThread = getConfig("max_stack_trace_lines_per_thread", ob, numericalConfigurationDefaultValues.MAX_STACKTRACE_LINES_PER_THREAD);
            this.maxStackTraceLineLength = getConfig("max_stack_trace_line_length", ob, numericalConfigurationDefaultValues.MAX_STACKTRACE_LINE_LENGTH);
            ...
            if (this.ignore_prefetch) {
                log(logLevelEnums.DEBUG, "initialize, ignoring prefetching and pre-rendering from counting as real website visits :[" + this.ignore_prefetch + "]");
            }
            ...
            if (inactivityTime !== numericalConfigurationDefaultValues.INACTIVITY_TIME) {
            ...
            if (sessionUpdate !== numericalConfigurationDefaultValues.SESSION_UPDATE) {
            ...
            if (maxEventBatch !== numericalConfigurationDefaultValues.MAX_EVENT_BATCH) {
            ...
            if (sessionCookieTimeout !== numericalConfigurationDefaultValues.SESSION_COOKIE_TIMEOUT) {
