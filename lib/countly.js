    ...
    var CountlyClass = function(ob) {
        ...
        * */
        ...
        function prepareRequest(request) {
            ...
            var uaVal = null;
            if (request.metrics) { // if metrics was provided parse its value
                uaVal = JSON.parse(request.metrics)._ua;
            }

            if (!request.metrics || !uaVal) { // if metrics or ua was not provided pass useragent with this event
                var metrics = getMetrics();
                request.metrics = JSON.stringify({ _ua: metrics._ua });
            }
