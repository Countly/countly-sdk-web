    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        this.initialize = function() {
            ...
            pastRating = setGetValueInStorage("cly_past_rating") || [];
            pastSurvey = setGetValueInStorage("cly_past_survey") || [];
            pastNps = setGetValueInStorage("cly_past_nps") || [];
        ...
        this.change_id = function(newId, merge) {
            ...
            if (this.device_id !== newId) {
                ...
                if (merge) {
                    ...
                    if (pastRating.includes(oldId)) {
                        pastRating.push(newId);
                        setGetValueInStorage("cly_past_rating", pastRating);
                    }
                    if (pastSurvey.includes(oldId)) {
                        pastSurvey.push(newId);
                        setGetValueInStorage("cly_past_survey", pastSurvey);
                    }
                    if (pastNps.includes(oldId)) {
                        pastNps.push(newId);
                        setGetValueInStorage("cly_past_nps", pastNps);
                    }
                }
                else {
        ...
        * */
        this.recordRatingWidgetWithID = function(ratingWidget) {
            ...
            if (pastRating.includes(this.device_id)) {
                log(logLevelEnums.ERROR, "recordRatingWidgetWithID, Rating has already been reported for this device/user");
                return;
            }
            ...
            add_cly_events(event);
            pastRating.push(this.device_id);
            setGetValueInStorage("cly_past_rating", pastRating);
        };
        /**
        ...
        * 
        * */
        this.recordSurveyNpsWithID = function(type, widget) {
            log(logLevelEnums.INFO, "recordSurveyNpsWithID, Providing information about user from: [" + type + "] with ID: [ " + widget.widget_id + " ]");
            ...
            if (type !== "nps" && type !== "survey") {
                log(logLevelEnums.ERROR, "recordSurveyNpsWithID, Entered type [" + type + "] must be 'survey' or 'nps'");
                return;
            }
            if (!widget.widget_id) {
                log(logLevelEnums.ERROR, "recordSurveyNpsWithID, Feedback Widgets must contain widget_id property");
                return;
            }
            ...
            if (type === "nps") {
                if (!widget.rating) {
                    log(logLevelEnums.ERROR, "recordSurveyNpsWithID, Widget must contain rating property");
                    return;
                }
                if (pastNps.includes(this.device_id)) {
                    log(logLevelEnums.ERROR, "recordSurveyNpsWithID, Widget has already been reported for this device/user");
                    return;
                }
                props = ["widget_id", "platform", "app_version", "rating", "comment"];
            }
            else if (type === "survey") {
                if (widget.answers.length < 1 || !answerValidator(widget.answers)) {
                    log(logLevelEnums.ERROR, "recordSurveyNpsWithID, Widget should have answers to be reported");
                    return;
                }
                if (pastSurvey.includes(this.device_id)) {
                    log(logLevelEnums.ERROR, "recordSurveyNpsWithID, Widget has already been reported for this device/user");
                    return;
                }
                props = ["widget_id", "platform", "app_version"];
            }
            ...
            // add event
            log(logLevelEnums.INFO, "recordSurveyNpsWithID, Reporting " + type + ": ", event);
            add_cly_events(event);

            // record this action so it wont be repeated
            if (type === "nps") {
                pastNps.push(this.device_id);
                setGetValueInStorage("cly_past_nps", pastNps);
            }
            else {
                pastSurvey.push(this.device_id);
                setGetValueInStorage("cly_past_survey", pastSurvey);
            }
        };
        /**
