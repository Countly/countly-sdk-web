...
(function(root, factory) {
    ...
    var CountlyClass = function(ob) {
        ...
        * */
        ...
        function add_cly_events(event) {
            ...
            log(logLevelEnums.INFO, "Successfully adding last event: ", e);
        }
    ...
    * */
    ...
    function secureRandom() {
        ...
        // eslint-disable-next-line no-undef
        if (cryptoObj && cryptoObj.getRandomValues && Uint8Array) {
        // eslint-disable-next-line no-undef
            id = id.replace(/[x]/g, function(c) {
                // eslint-disable-next-line no-undef
                var random = crypto.getRandomValues(new Uint8Array(6))[0];
                var result = (random & 0xf).toString(16);
                return (c === "x" ? result : (result & 0x3 | 0x8)).toString(16);
            });
        }
        else { // if no crypto support or ES5 polyfill, use Math.random
            id = id.replace(/[x]/g, function(c) {
    ...
    function generateUUID() {
        ...
        // use crypto if available to generate random numbers
        // eslint-disable-next-line no-undef
        if (cryptoObj && cryptoObj.getRandomValues && Uint8Array) {
