        ...
        if (enableWidgets.length > 0) {
            ...
            sendXmlHttpRequest(Countly.url + '/o/feedback/multiple-widgets-by-id', {app_key:Countly.app_key, widgets:JSON.stringify(params.widgets)}, function(err, params, responseText){
                ...
                try {
                    ...
                    for (var i = 0; i < widgets.length; i++) {
                        var pages = target_pages[i];
                        for (var k = 0; k < pages.length; k++) {
                            if (pages[k] === window.location.pathname && !widgets[i].hide_sticker) {
                                (function(currentWidgetId) { 
                                    add_event(document.getElementById('countly-feedback-sticker-'+currentWidgetId), 'click', function(){document.getElementById('countly-iframe-wrapper-'+currentWidgetId).style.display = "block";document.getElementById('cfbg').style.display = "block";});
                                    add_event(document.getElementById('countly-feedback-close-icon-'+currentWidgetId), 'click', function(){document.getElementById('countly-iframe-wrapper-'+currentWidgetId).style.display = "none";document.getElementById('cfbg').style.display = "none";});
                                })(widgets[i]._id);
                            }
                        }
                    }   
