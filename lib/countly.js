    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        * */
            ...
            try {
                ...
                xhr.onreadystatechange = function() {
                    if (this.readyState === 4) {
                        ...
                        if ((functionName === "initializeRatingWidgets") || (functionName === "presentRatingWidgetWithID")) {
                            isResponseValidated = isResponseValidBroad(this.status, this.responseText, ignoreResultField);
                        }
                        else {
                            isResponseValidated = isResponseValid(this.status, this.responseText, ignoreResultField);
                        }
         ...
         * @returns {Boolean} - returns true if response passes the tests 
         */
        function isResponseValidBroad(statusCode, str, ignoreResultField) {
            // status code and response format check
            if (!(statusCode >= 200 && statusCode < 300)) {
                log(logLevelEnums.ERROR, "Http response status code is not within the expected range: ", statusCode);
                return false;
