    ...
    Countly.init = function(ob){
        if(!inited){
            ...
            if(window.name && window.name.indexOf("cly:") == 0){
                passed_data = JSON.parse(window.name.replace("cly:", ""));
            }
            else if(location.hash === "#cly:"){
                passed_data = JSON.parse(location.hash.replace("#cly:", ""));
            }
            
            if(passed_data){
                if(passed_data.token && passed_data.purpose){
                    if(passed_data.token != store("cly_old_token")){
                        setToken(passed_data.token);
                        store("cly_old_token", passed_data.token);
                    }
                    if(passed_data.purpose === "heatmap"){
                        ignoreVisitor = true;
                        loadJS(Countly.url+"/views/heatmap.js");
                    }
                }
            }
    ...
    **/
    ...
    function heartBeat(){
        ...
        if(typeof Countly.onload !== "undefined" && Countly.onload.length > 0){
            for(var i = 0; i < Countly.onload.length; i++){
