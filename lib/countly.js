    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        this.initialize = function() {
            ...
            // check device ID precedence: URL > STORED > DEVELOPER > TEMP > AUTO
            // first checking if the stored ID was temp ID
            if (setGetValueInStorage("cly_id") === "[CLY]_temp_id") {
                // check if device ID was provided with URL
                if (deviceIdParamValue !== null) {
                ...
                // if not check id user also provided an ID
                else if (getConfig("device_id", ob, undefined)) {
                    this.device_id = getConfig("device_id", ob, undefined);
                ...
                // if not turn on the offline mode and keep the ID as temp
                else {
                    offlineMode = true;
                    this.device_id = "[CLY]_temp_id";
                    deviceIdType = DeviceIdTypeInternalEnums.TEMPORARY_ID;
                    log(logLevelEnums.INFO, "initialize, Extends the offline mode from previous session");
                }
            }
            // if the stored ID was not temp ID or nothing was stored
            else {
                // check if there wqs stored ID
                if (setGetValueInStorage("cly_id")) {
                ...
                // if not check if offline mode is on
                    ...
                    this.device_id = "[CLY]_temp_id";
                    deviceIdType = DeviceIdTypeInternalEnums.TEMPORARY_ID;
                    log(logLevelEnums.INFO, "initialize, Offline mode enabled");
                }

                // if all fails generate an ID
                else {
                    log(logLevelEnums.INFO, "initialize, No stored ID");
