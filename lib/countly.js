    ...
    var CountlyClass = function(ob) {
        ...
        this.initialize = function() {
            ...
            if (maxCrashLogs && !this.maxBreadcrumbCount) {
                ...
                log(logLevelEnums.WARNING, "[Initialization] 'maxCrashLogs' is deprecated. Use 'maxBreadcrumbCount' instead!");
            }
            ...
            if (!this.rcAutoOptinAb && !this.useExplicitRcApi) {
                log(logLevelEnums.WARNING, "[Initialization] Auto opting is disabled, switching to explicit RC API");
            ...
            if (this.url === "") {
                log(logLevelEnums.ERROR, "[Initialization] Please provide server URL");
            ...
            if (this.clearStoredId) {
                ...
                if (getValueFromStorage("cly_id") && !tempIdModeWasEnabled) {
                    ...
                    log(logLevelEnums.DEBUG, "[Initialization] temporarily using the previous device ID to flush existing events");
                    deviceIdType = getValueFromStorage("cly_id_type");
                    if (!deviceIdType) {
                        log(logLevelEnums.DEBUG, "[Initialization] No device ID type info from the previous session, falling back to DEVELOPER_SUPPLIED, for event flushing");
                ...
                log(logLevelEnums.INFO, "[Initialization] Clearing the device ID storage");
            ...
            if (window.name && window.name.indexOf("cly:") === 0) {
                try {
                    ...
                    log(logLevelEnums.ERROR, "[Initialization] Could not parse name: " + window.name + ", error: " + ex);
                }
            }
                ...
                try {
                    ...
                    log(logLevelEnums.ERROR, "[Initialization] Could not parse hash: " + location.hash + ", error: " + ex);
                }
            }
            ...
            if (this.ignore_visitor) {
                log(logLevelEnums.WARNING, "[Initialization] ignore_visitor:[" + this.ignore_visitor + "], this user will not be tracked");
                return;
            }

            // init configuration is printed out here:
            // key values should be printed out as is
            log(logLevelEnums.DEBUG, "[Initialization] app_key:[" + this.app_key + "], url:[" + this.url + "]");
            log(logLevelEnums.DEBUG, "[Initialization] device_id:[" + getConfig("device_id", ob, undefined) + "]");
            log(logLevelEnums.DEBUG, "[Initialization] require_consent is enabled:[" + this.require_consent + "]");
            try {
                log(logLevelEnums.DEBUG, "[Initialization] metric override:[" + JSON.stringify(this.metrics) + "]");
                log(logLevelEnums.DEBUG, "[Initialization] header override:[" + JSON.stringify(this.headers) + "]");
                ...
                log(logLevelEnums.DEBUG, "[Initialization] utm tags:[" + JSON.stringify(this.utm) + "]");
                // empty array printed if non provided
                if (ignoreReferrers) {
                    log(logLevelEnums.DEBUG, "[Initialization] referrers to ignore :[" + JSON.stringify(ignoreReferrers) + "]");
                }
            }
            catch (e) {
                log(logLevelEnums.ERROR, "[Initialization] Could not stringify some config object values");
            }
            log(logLevelEnums.DEBUG, "[Initialization] app_version:[" + this.app_version + "]");

            // location info printed here
            log(logLevelEnums.DEBUG, "[Initialization] provided location info; country_code:[" + this.country_code + "], city:[" + this.city + "], ip_address:[" + this.ip_address + "]");
            ...
            if (this.clearStoredId) {
                log(logLevelEnums.DEBUG, "[Initialization] clearStoredId flag set to:[" + this.clearStoredId + "]");
            }
            if (this.ignore_prefetch) {
                log(logLevelEnums.DEBUG, "[Initialization] ignoring pre-fetching and pre-rendering from counting as real website visits :[" + this.ignore_prefetch + "]");
            }
            // if test mode is enabled warn the user
            if (this.test_mode) {
                log(logLevelEnums.WARNING, "[Initialization] test_mode:[" + this.test_mode + "], request queue won't be processed");
            }
            if (this.test_mode_eq) {
                log(logLevelEnums.WARNING, "[Initialization] test_mode_eq:[" + this.test_mode_eq + "], event queue won't be processed");
            }
            // if test mode is enabled warn the user
            if (this.heatmapWhitelist) {
                log(logLevelEnums.DEBUG, "[Initialization] heatmap whitelist:[" + JSON.stringify(this.heatmapWhitelist) + "], these domains will be whitelisted");
            }
            // if storage is se to something other than local storage
            if (this.storage !== "default") {
                log(logLevelEnums.DEBUG, "[Initialization] storage is set to:[" + this.storage + "]");
            }
            if (this.ignore_bots) {
                log(logLevelEnums.DEBUG, "[Initialization] ignore traffic from bots :[" + this.ignore_bots + "]");
            }
            if (this.force_post) {
                log(logLevelEnums.DEBUG, "[Initialization] forced post method for all requests:[" + this.force_post + "]");
            }
            if (this.remote_config) {
                log(logLevelEnums.DEBUG, "[Initialization] remote_config callback provided:[" + !!this.remote_config + "]");
            }
            if (typeof this.rcAutoOptinAb === "boolean") {
                log(logLevelEnums.DEBUG, "[Initialization] automatic RC optin is enabled:[" + this.rcAutoOptinAb + "]");
            }
            if (!this.useExplicitRcApi) {
                log(logLevelEnums.WARNING, "[Initialization] will use legacy RC API. Consider enabling new API during init with use_explicit_rc_api flag");
            }
            if (this.track_domains) {
                log(logLevelEnums.DEBUG, "[Initialization] tracking domain info:[" + this.track_domains + "]");
            }
            ...
            if (!useSessionCookie) {
                log(logLevelEnums.WARNING, "[Initialization] use_session_cookie is enabled:[" + useSessionCookie + "]");
            }
            if (offlineMode) {
                log(logLevelEnums.DEBUG, "[Initialization] offline_mode:[" + offlineMode + "], user info won't be send to the servers");
            }
            if (offlineMode) {
                log(logLevelEnums.DEBUG, "[Initialization] stored remote configs:[" + JSON.stringify(remoteConfigs) + "]");
            }
            // functions, if provided, would be printed as true without revealing their content
            log(logLevelEnums.DEBUG, "[Initialization] 'getViewName' callback override provided:[" + (this.getViewName !== Countly.getViewName) + "]");
            log(logLevelEnums.DEBUG, "[Initialization] 'getSearchQuery' callback override provided:[" + (this.getSearchQuery !== Countly.getSearchQuery) + "]");

            // limits are printed here if they were modified 
            if (this.maxKeyLength !== configurationDefaultValues.MAX_KEY_LENGTH) {
                log(logLevelEnums.DEBUG, "[Initialization] maxKeyLength set to:[" + this.maxKeyLength + "] characters");
            }
            if (this.maxValueSize !== configurationDefaultValues.MAX_VALUE_SIZE) {
                log(logLevelEnums.DEBUG, "[Initialization] maxValueSize set to:[" + this.maxValueSize + "] characters");
            }
            if (this.maxSegmentationValues !== configurationDefaultValues.MAX_SEGMENTATION_VALUES) {
                log(logLevelEnums.DEBUG, "[Initialization] maxSegmentationValues set to:[" + this.maxSegmentationValues + "] key/value pairs");
            }
            if (this.maxBreadcrumbCount !== configurationDefaultValues.MAX_BREADCRUMB_COUNT) {
                log(logLevelEnums.DEBUG, "[Initialization] maxBreadcrumbCount for custom logs set to:[" + this.maxBreadcrumbCount + "] entries");
            }
            if (this.maxStackTraceLinesPerThread !== configurationDefaultValues.MAX_STACKTRACE_LINES_PER_THREAD) {
                log(logLevelEnums.DEBUG, "[Initialization] maxStackTraceLinesPerThread set to:[" + this.maxStackTraceLinesPerThread + "] lines");
            }
            if (this.maxStackTraceLineLength !== configurationDefaultValues.MAX_STACKTRACE_LINE_LENGTH) {
                log(logLevelEnums.DEBUG, "[Initialization] maxStackTraceLineLength set to:[" + this.maxStackTraceLineLength + "] characters");
            }
            if (beatInterval !== configurationDefaultValues.BEAT_INTERVAL) {
                log(logLevelEnums.DEBUG, "[Initialization] interval for heartbeats set to:[" + beatInterval + "] milliseconds");
            }
            if (queueSize !== configurationDefaultValues.QUEUE_SIZE) {
                log(logLevelEnums.DEBUG, "[Initialization] queue_size set to:[" + queueSize + "] items max");
            }
            if (failTimeoutAmount !== configurationDefaultValues.FAIL_TIMEOUT_AMOUNT) {
                log(logLevelEnums.DEBUG, "[Initialization] fail_timeout set to:[" + failTimeoutAmount + "] seconds of wait time after a failed connection to server");
            }
            if (inactivityTime !== configurationDefaultValues.INACTIVITY_TIME) {
                log(logLevelEnums.DEBUG, "[Initialization] inactivity_time set to:[" + inactivityTime + "] minutes to consider a user as inactive after no observable action");
            }
            if (sessionUpdate !== configurationDefaultValues.SESSION_UPDATE) {
                log(logLevelEnums.DEBUG, "[Initialization] session_update set to:[" + sessionUpdate + "] seconds to check if extending a session is needed while the user is active");
            }
            if (maxEventBatch !== configurationDefaultValues.MAX_EVENT_BATCH) {
                log(logLevelEnums.DEBUG, "[Initialization] max_events set to:[" + maxEventBatch + "] events to send in one batch");
            }
            if (maxCrashLogs) {
                log(logLevelEnums.WARNING, "[Initialization] max_logs set to:[" + maxCrashLogs + "] breadcrumbs to store for crash logs max, deprecated ");
            }
            if (sessionCookieTimeout !== configurationDefaultValues.SESSION_COOKIE_TIMEOUT) {
                log(logLevelEnums.DEBUG, "[Initialization] session_cookie_timeout set to:[" + sessionCookieTimeout + "] minutes to expire a cookies session");
            }

            log(logLevelEnums.INFO, "[Initialization] Countly initialized");
            ...
            if (getValueFromStorage("cly_id") && !tempIdModeWasEnabled) {
                this.device_id = getValueFromStorage("cly_id");
                log(logLevelEnums.INFO, "[Initialization] Set the stored device ID");
                deviceIdType = getValueFromStorage("cly_id_type");
                if (!deviceIdType) {
            ...
            // if not check if device ID was provided with URL
                ...
                log(logLevelEnums.INFO, "[Initialization] Device ID set by URL");
            ...
            // if not check if developer provided any ID
                ...
                log(logLevelEnums.INFO, "[Initialization] Device ID set by developer");
            ...
            // if not check if offline mode is on
                ...
                if (offlineMode && tempIdModeWasEnabled) {
                    log(logLevelEnums.INFO, "[Initialization] Temp ID set, continuing offline mode from previous app session");
                }
                else if (offlineMode && !tempIdModeWasEnabled) {
                    ...
                    log(logLevelEnums.INFO, "[Initialization] Temp ID set, entering offline mode");
                }
                else {
                    ...
                    offlineMode = true;
                    log(logLevelEnums.INFO, "[Initialization] Temp ID set, enabling offline mode");
                }
            }
            // if all fails generate an ID
            else {
                log(logLevelEnums.INFO, "[Initialization] Generating the device ID");
        ...
        */
        ...
        var updateConsent = function() {
            ...
            consentTimer = setTimeout(function() {
                ...
                log(logLevelEnums.DEBUG, "[updateConsent] Consent update request has been sent to the queue.");
            }, 1000);
        };
        ...
        * */
        ...
        function add_cly_events(event, eventIdOverride) {
            ...
            if (self.ignore_visitor) {
                log(logLevelEnums.ERROR, "[add_event] Adding event failed. Possible bot or user opt out");
                return;
            }

            if (!event.key) {
                log(logLevelEnums.ERROR, "[add_event] Adding event failed. Event must have a key property");
                return;
            }
            ...
            log(logLevelEnums.INFO, "[add_event] With event ID: [" + e.id + "], successfully adding the last event:", e);
        }
        ...
        * */
        ...
        function processWidget(currentWidget, hasSticker) {
            ...
            if (isDuplicate) {
                log(logLevelEnums.ERROR, "[FeedbackWidget] Widget with same ID exists");
                return;
            }
            try {
                // create wrapper div
                ...
                log(logLevelEnums.ERROR, "[FeedbackWidget] Somethings went wrong while element injecting process: " + e);
            }
        }
        ...
        function toRequestQueue(request) {
            if (self.ignore_visitor) {
                log(logLevelEnums.WARNING, "[RequestQueue] User is opt_out will ignore the request: " + request);
                return;
            }

            if (!self.app_key || !self.device_id) {
                log(logLevelEnums.ERROR, "[RequestQueue] app_key or device_id is missing ", self.app_key, self.device_id);
                return;
            }
        ...
        function heartBeat() {
            ...
            if (self.ignore_visitor) {
                hasPulse = false;
                log(logLevelEnums.WARNING, "[HeartBeat] User opt_out, no heartbeat");
                return;
            }
            ...
            if (global && typeof Countly.q !== "undefined" && Countly.q.length > 0) {
                ...
                for (i = 0; i < q.length; i++) {
                    req = q[i];
                    log(logLevelEnums.DEBUG, "[HeartBeat] Processing queued call", req);
            ...
            if (!offlineMode && requestQueue.length > 0 && readyToProcess && getTimestamp() > failTimeout) {
                ...
                log(logLevelEnums.DEBUG, "[HeartBeat] Processing request", params);
                setValueInStorage("cly_queue", requestQueue, true);
                if (!self.test_mode) {
        ...
        function getMetrics() {
                        ...
                        for (var k = 0; k < ignoreReferrers.length; k++) {
                            try {
                                ...
                                if (reg.test(document.referrer)) {
                                    log(logLevelEnums.DEBUG, "[Metrics] Ignored: " + document.referrer);
                                    ignoring = true;
                                    break;
                                }
                            }
                            catch (ex) {
                                log(logLevelEnums.ERROR, "[Metrics] Problem with ignoring: " + ignoreReferrers[k], ", error: " + ex);
                            }
                        }
                    }
                    if (!ignoring) {
            ...
            log(logLevelEnums.DEBUG, "[Metrics] Got metrics", metrics);
            return metrics;
        }
        ...
        function sendXmlHttpRequest(functionName, url, params, callback, useBroadResponseValidator) {
            ...
            try {
                log(logLevelEnums.DEBUG, "[sendXmlHttpRequest] Sending XML HTTP request");
                var xhr = null;
                ...
                xhr.onreadystatechange = function() {
                    if (this.readyState === 4) {
                        log(logLevelEnums.DEBUG, functionName + " [sendXmlHttpRequest] HTTP request completed [" + this.status + "][" + this.responseText + "]");
                ...
                // fallback
                log(logLevelEnums.ERROR, functionName + " [sendXmlHttpRequest] Failed XML HTTP request: " + e);
                if (typeof callback === "function") {
        ...
        function isResponseValid(statusCode, str) {
            ...
            try {
                ...
                if (Object.prototype.toString.call(parsedResponse) !== "[object Object]") {
                    log(logLevelEnums.ERROR, "[ResponseValidation] Http response is not JSON Object");
                    return false;
                }
                ...
                log(logLevelEnums.ERROR, "[ResponseValidation] Http response is not JSON: " + e);
                return false;
            }
        }
        ...
        function isResponseValidBroad(statusCode, str) {
            ...
            try {
                ...
                if ((Object.prototype.toString.call(parsedResponse) !== "[object Object]") && (!Array.isArray(parsedResponse))) {
                    log(logLevelEnums.ERROR, "[ResponseValidationBroad] Http response is not JSON Object nor JSON Array");
                    return false;
                }
                ...
                log(logLevelEnums.ERROR, "[ResponseValidationBroad] Http response is not JSON: " + e);
                return false;
            }
        }
        ...
        */
        ...
        function getValueFromStorage(key, useLocalStorage, useRawKey) {
            ...
            if (self.storage === "none") {
                log(logLevelEnums.WARNING, "[Storage] Storage is disabled. Value with key: " + key + " won't be retrieved");
                return;
            }
        ...
        */
        function setValueInStorage(key, value, useLocalStorage, useRawKey) {
            ...
            if (self.storage === "none") {
                log(logLevelEnums.WARNING, "[Storage] Storage is disabled. Value with key: " + key + " won't be stored");
                return;
            }
        ...
        */
        function removeValueFromStorage(key, useLocalStorage, useRawKey) {
            // check if we should use storage options at all
            if (self.storage === "none") {
                log(logLevelEnums.WARNING, "[Storage] Storage is disabled. Value with key: " + key + " won't be removed");
                return;
            }
        ...
        this.onStorageChange = function(key, newValue) {
            log(logLevelEnums.INFO, "[Storage] onStorageChange, Applying storage changes");
            switch (key) {
            // queue of requests
            case "cly_queue":
