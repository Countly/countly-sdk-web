    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        this.initialize = function() {
            ...
            this.rcOptin = getConfig("rc_optin", ob, true);
            this.useRcApi = getConfig("use_new_rc_api", ob, false);
            ...
            if (typeof this.rcOptin === "boolean") {
                log(logLevelEnums.DEBUG, "initialize, automatic RC optin is enabled:[" + this.rcOptin + "]");
            }
            if (!this.useRcApi) {
                log(logLevelEnums.WARNING, "initialize, will use legacy RC API. Consider enabling new API during init with use_new_rc_api flag");
            }
        ...
        this.halt = function() {
            ...
            self.rcOptin = undefined;
            self.useRcApi = undefined;
        ...
        * */
        this.fetch_remote_config = function(keys, omit_keys, callback) {
            ...
            if (this.useRcApi) {
                ...
                fetch_remote_config_opt(keys, omit_keys, opt, callback);
                return;
            }

            log(logLevelEnums.WARNING, "fetch_remote_config, Fetching remote config, with legacy API");
            fetch_remote_config_opt(keys, omit_keys, "legacy", callback);
        };
        ...
        * */
        function fetch_remote_config_opt(keys, omit_keys, optIn, api, callback) {
            log(logLevelEnums.INFO, "fetch_remote_config_opt, Fetching sequence initiated");
            var request = {
                method: "rc"
            };
     ...
     * @param {boolean} [conf.rc_optin=true] - opts in the user for A/B testing while fetching the remote config (if true)
     * @param {boolean} [conf.use_new_rc_api=false] - set it to true to use the new remote config API
