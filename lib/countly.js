    ...
    */
    ...
    var CountlyClass = /*#__PURE__*/_createClass(function CountlyClass(ob) {
        ...
        * */
        ...
        /**
         *  Add 'checksum256' param for the request, when set up the salt
         *  @memberof Countly._internals
         *  @param {String} paramsString - URL query string
         *  @returns {String} if set up the salt, return URL query string with 'checksum256' param; if not set up,then return the origin URL query string
         */
        function addCheckSumParam(paramsString) {
        ...
        function sendXmlHttpRequest(functionName, url, params, callback, useBroadResponseValidator) {
            ...
            try {
                ...
                var withCheckSumParamData = addCheckSumParam(data);
                var method = "GET";
                if (self.force_post || withCheckSumParamData.length >= 2000) {
                    method = "POST";
                }
                if (method === "GET") {
                    xhr.open("GET", url + "?" + withCheckSumParamData, true);
                } else {
                ...
                if (method === "GET") {
                    ...
                    xhr.send(withCheckSumParamData);
                }
            } catch (e) {
                // fallback
        ...
        function sendFetchRequest(functionName, url, params, callback, useBroadResponseValidator) {
            ...
            try {
                ...
                params = params || {};
                var withCheckSumParam = addCheckSumParam(prepareParams(params));
                if (self.force_post || withCheckSumParam.length >= 2000) {
                    method = "POST";
                } else {
                    url += "?" + withCheckSumParam;
                }
                ...
                fetch(url, {
                    method: method,
                    headers: headers,
                    body: method === "POST" ? withCheckSumParam : null,
                }).then(function (res) {
                    response = res;
