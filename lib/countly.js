...
(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    : ((global =
      ...
      factory((global.Countly = global.Countly || {})));
  ...
  function truncateObject(
    ...
    if (obj) {
      ...
      for (var key in obj) {
        ...
        var newValue = truncateSingleValue(
          obj[key],
          valueLimit,
          errorLog,
          logCall
  ...
  function add_event_listener(element, type, listener) {
    ...
    if (element === null || typeof element === "undefined") {
      ...
      if (checkIfLoggingIsOn()) {
        ...
        console.warn(
          "[WARNING] [Countly] add_event_listener, Can't bind [" +
            type +
            "] event to nonexisting element"
        );
      }
      return;
    }
  ...
  function showLoader() {
    ...
    if (!loader) {
      var css =
        "#cly-loader {height: 4px; width: 100%; position: absolute; z-index: 99999; overflow: hidden; background-color: #fff; top:0px; left:0px;}" +
      ...
      document.body.onload = function () {
        ...
        if (Countly.showLoaderProtection) {
          if (checkIfLoggingIsOn()) {
            // eslint-disable-next-line no-console
            console.warn(
              "[WARNING] [Countly] showloader, Loader is already on"
            );
          }
          return;
        }
        try {
          ...
          if (checkIfLoggingIsOn()) {
            // eslint-disable-next-line no-console
            console.error(
              "[ERROR] [Countly] showLoader, Body is not loaded for loader to append: " +
                e
            );
          }
        }
  ...
  var CountlyClass = /*#__PURE__*/ _createClass(function CountlyClass(ob) {
    ...
    var apiPath = "/i";
    var readPath = "/o/sdk";
    var beatInterval = getConfig(
      "interval",
      ob,
      configurationDefaultValues.BEAT_INTERVAL
    );
    var queueSize = getConfig(
      "queue_size",
      ob,
      configurationDefaultValues.QUEUE_SIZE
    );
    var requestQueue = [];
    var eventQueue = [];
    var remoteConfigs = {};
    ...
    var failTimeout = 0;
    var failTimeoutAmount = getConfig(
      "fail_timeout",
      ob,
      configurationDefaultValues.FAIL_TIMEOUT_AMOUNT
    );
    var inactivityTime = getConfig(
      "inactivity_time",
      ob,
      configurationDefaultValues.INACTIVITY_TIME
    );
    var inactivityCounter = 0;
    var sessionUpdate = getConfig(
      "session_update",
      ob,
      configurationDefaultValues.SESSION_UPDATE
    );
    var maxEventBatch = getConfig(
      "max_events",
      ob,
      configurationDefaultValues.MAX_EVENT_BATCH
    );
    var maxCrashLogs = getConfig("max_logs", ob, null);
    var useSessionCookie = getConfig("use_session_cookie", ob, true);
    var sessionCookieTimeout = getConfig(
      "session_cookie_timeout",
      ob,
      configurationDefaultValues.SESSION_COOKIE_TIMEOUT
    );
    ...
    try {
      ...
      log(
        logLevelEnums.ERROR,
        "Local storage test failed, Halting local storage support: " + e
      );
      lsSupport = false;
    }
    ...
    this.initialize = function () {
      ...
      this.getSearchQuery = getConfig(
        "getSearchQuery",
        ob,
        Countly.getSearchQuery
      );
      ...
      this.salt = getConfig("salt", ob, undefined);
      ...
      this.track_domains = !isBrowser
        ? undefined
        : getConfig("track_domains", ob, true);
      this.storage = getConfig("storage", ob, "default");
      this.enableOrientationTracking = !isBrowser
        ? undefined
        : getConfig("enable_orientation_tracking", ob, true);
      this.maxKeyLength = getConfig(
        "max_key_length",
        ob,
        configurationDefaultValues.MAX_KEY_LENGTH
      );
      this.maxValueSize = getConfig(
        "max_value_size",
        ob,
        configurationDefaultValues.MAX_VALUE_SIZE
      );
      this.maxSegmentationValues = getConfig(
        "max_segmentation_values",
        ob,
        configurationDefaultValues.MAX_SEGMENTATION_VALUES
      );
      this.maxBreadcrumbCount = getConfig("max_breadcrumb_count", ob, null);
      this.maxStackTraceLinesPerThread = getConfig(
        "max_stack_trace_lines_per_thread",
        ob,
        configurationDefaultValues.MAX_STACKTRACE_LINES_PER_THREAD
      );
      this.maxStackTraceLineLength = getConfig(
        "max_stack_trace_line_length",
        ob,
        configurationDefaultValues.MAX_STACKTRACE_LINE_LENGTH
      );
      this.heatmapWhitelist = getConfig("heatmap_whitelist", ob, []);
      self.hcErrorCount =
        getValueFromStorage(healthCheckCounterEnum.errorCount) || 0;
      self.hcWarningCount =
        getValueFromStorage(healthCheckCounterEnum.warningCount) || 0;
      self.hcStatusCode =
        getValueFromStorage(healthCheckCounterEnum.statusCode) || -1;
      self.hcErrorMessage =
        getValueFromStorage(healthCheckCounterEnum.errorMessage) || "";
      ...
      if (this.clearStoredId) {
        ...
        if (getValueFromStorage("cly_id") && !tempIdModeWasEnabled) {
          ...
          log(
            logLevelEnums.DEBUG,
            "initialize, temporarily using the previous device ID to flush existing events"
          );
          deviceIdType = getValueFromStorage("cly_id_type");
          if (!deviceIdType) {
            log(
              logLevelEnums.DEBUG,
              "initialize, No device ID type info from the previous session, falling back to DEVELOPER_SUPPLIED, for event flushing"
            );
      ...
      if (
        ...
        (this.passed_data && !this.passed_data.app_key && global)
      ) {
      ...
      if (sdkName === SDK_NAME && sdkVersion === SDK_VERSION) {
        log(
          logLevelEnums.DEBUG,
          "initialize, SDK name:[" + sdkName + "], version:[" + sdkVersion + "]"
        );
      } else {
        log(
          logLevelEnums.DEBUG,
      ...
      log(
        logLevelEnums.DEBUG,
        "initialize, device_id:[" + getConfig("device_id", ob, undefined) + "]"
      );
      log(
        logLevelEnums.DEBUG,
        "initialize, require_consent is enabled:[" + this.require_consent + "]"
      );
      try {
        log(
          logLevelEnums.DEBUG,
          "initialize, metric override:[" + JSON.stringify(this.metrics) + "]"
        );
        log(
          logLevelEnums.DEBUG,
          "initialize, header override:[" + JSON.stringify(this.headers) + "]"
        );
        ...
        if (ignoreReferrers) {
          log(
            logLevelEnums.DEBUG,
            "initialize, referrers to ignore :[" +
              JSON.stringify(ignoreReferrers) +
              "]"
          );
        }
      } catch (e) {
      ...
      log(
        logLevelEnums.DEBUG,
        "initialize, app_version:[" + this.app_version + "]"
      );
      ...
      if (this.namespace !== "") {
        log(
          logLevelEnums.DEBUG,
          "initialize, namespace given:[" + this.namespace + "]"
        );
      }
      if (this.clearStoredId) {
        log(
          logLevelEnums.DEBUG,
          "initialize, clearStoredId flag set to:[" + this.clearStoredId + "]"
        );
      }
      ...
      if (this.test_mode) {
        log(
          ...
          "initialize, test_mode:[" +
            this.test_mode +
            "], request queue won't be processed"
        );
      }
      if (this.test_mode_eq) {
        log(
          logLevelEnums.WARNING,
          "initialize, test_mode_eq:[" +
            this.test_mode_eq +
            "], event queue won't be processed"
        );
      }
      // if test mode is enabled warn the user
      ...
      if (this.storage !== "default") {
        log(
          logLevelEnums.DEBUG,
          "initialize, storage is set to:[" + this.storage + "]"
        );
      }
      if (this.ignore_bots) {
        log(
      ...
      if (!useSessionCookie) {
        log(
          logLevelEnums.WARNING,
          "initialize, use_session_cookie is enabled:[" + useSessionCookie + "]"
        );
      }
      if (offlineMode) {
        log(
      ...
      if (offlineMode) {
        log(
          logLevelEnums.DEBUG,
          "initialize, stored remote configs:[" +
            JSON.stringify(remoteConfigs) +
            "]"
        );
      }
      ...
      log(
        ...
        "initialize, 'getViewName' callback override provided:[" +
          (this.getViewName !== Countly.getViewName) +
          "]"
      );
      log(
        logLevelEnums.DEBUG,
        "initialize, 'getSearchQuery' callback override provided:[" +
          (this.getSearchQuery !== Countly.getSearchQuery) +
          "]"
      );
      ...
      if (
        ...
        configurationDefaultValues.MAX_BREADCRUMB_COUNT
      ) {
        log(
          logLevelEnums.DEBUG,
      ...
      if (queueSize !== configurationDefaultValues.QUEUE_SIZE) {
        log(
          logLevelEnums.DEBUG,
          "initialize, queue_size set to:[" + queueSize + "] items max"
        );
      }
      if (
        failTimeoutAmount !== configurationDefaultValues.FAIL_TIMEOUT_AMOUNT
      ) {
        log(
          logLevelEnums.DEBUG,
      ...
      var tempIdModeWasEnabled =
        getValueFromStorage("cly_id") === "[CLY]_temp_id";
      ...
      // if not check if offline mode is on
        ...
        if (offlineMode && tempIdModeWasEnabled) {
          ...
          log(
            logLevelEnums.INFO,
            "initialize, Temp ID set, entering offline mode"
          );
        } else {
          ...
          log(
            logLevelEnums.INFO,
            "initialize, Temp ID set, enabling offline mode"
          );
        }
      }
    ...
    this.halt = function () {
      ...
      Countly.features = [
        featureEnums.SESSIONS,
        featureEnums.EVENTS,
        featureEnums.VIEWS,
        featureEnums.SCROLLS,
        featureEnums.CLICKS,
        featureEnums.FORMS,
        featureEnums.CRASHES,
        featureEnums.ATTRIBUTION,
        featureEnums.USERS,
        featureEnums.STAR_RATING,
        featureEnums.LOCATION,
        featureEnums.APM,
        featureEnums.FEEDBACK,
        featureEnums.REMOTE_CONFIG,
      ];
      ...
      if (features) {
        for (var i in features) {
          if (!consents[i]) {
            ...
            log(
              logLevelEnums.WARNING,
              "group_features, Feature name [" + i + "] is already reserved"
            );
          }
        }
      } else {
        log(
          logLevelEnums.ERROR,
          "group_features, Incorrect features:[" + features + "]"
        );
      }
    };
    ...
    this.check_consent = function (feature) {
      ...
      log(
        logLevelEnums.ERROR,
        "check_consent, No feature available for [" + feature + "]"
      );
      return false;
    };
    ...
    this.get_device_id = function () {
      log(
        logLevelEnums.INFO,
        "get_device_id, Retrieving the device id: [" + self.device_id + "]"
      );
    ...
    this.check_any_consent = function () {
      log(
        logLevelEnums.INFO,
        "check_any_consent, Checking if any consent is given"
      );
      if (!this.require_consent) {
    ...
    this.add_consent = function (feature) {
      log(
        logLevelEnums.INFO,
        "add_consent, Adding consent for [" + feature + "]"
      );
      if (Array.isArray(feature)) {
        ...
        if (consents[feature].features) {
          ...
          if (consents[feature].optin !== true) {
            ...
            setTimeout(function () {
              if (
                feature === featureEnums.SESSIONS &&
                lastParams.begin_session
              ) {
                self.begin_session.apply(self, lastParams.begin_session);
                lastParams.begin_session = null;
              } else if (
                feature === featureEnums.VIEWS &&
                lastParams.track_pageview
              ) {
                lastView = null;
        ...
        log(
          logLevelEnums.ERROR,
          "add_consent, No feature available for [" + feature + "]"
        );
      }
    };
    ...
    this.remove_consent = function (feature) {
      log(
        logLevelEnums.INFO,
        "remove_consent, Removing consent for [" + feature + "]"
      );
    ...
    this.remove_consent_internal = function (feature, enforceConsentUpdate) {
      ...
      if (Array.isArray(feature)) {
        ...
        if (consents[feature].features) {
          // this is added group, let's iterate through sub features
          this.remove_consent_internal(
            consents[feature].features,
        ...
        log(
          logLevelEnums.WARNING,
          "remove_consent, No feature available for [" + feature + "]"
        );
      }
    };
    var consentTimer;
    var updateConsent = function updateConsent() {
      ...
      consentTimer = setTimeout(function () {
        ...
        log(
          logLevelEnums.DEBUG,
          "Consent update request has been sent to the queue."
        );
      }, 1000);
    };
    ...
    this.disable_offline_mode = function (device_id) {
      if (!offlineMode) {
        log(
          logLevelEnums.WARNING,
          "disable_offline_mode, Countly was not in offline mode."
        );
        return;
      }
      ...
      if (device_id && this.device_id !== device_id) {
        ...
        setValueInStorage(
          "cly_id_type",
          DeviceIdTypeInternalEnums.DEVELOPER_SUPPLIED
        );
        log(
          logLevelEnums.INFO,
        ...
        if (this.device_id !== getValueFromStorage("cly_id")) {
          ...
          setValueInStorage(
            "cly_id_type",
            DeviceIdTypeInternalEnums.SDK_GENERATED
          );
        }
      }
    ...
    this.begin_session = function (noHeartBeat, force) {
      log(
        logLevelEnums.INFO,
        "begin_session, Starting the session. There was an ongoing session: [" +
          sessionStarted +
          "]"
      );
      if (noHeartBeat) {
      ...
      if (this.check_consent(featureEnums.SESSIONS)) {
        if (!sessionStarted) {
          ...
          log(
            logLevelEnums.VERBOSE,
            "begin_session, Session state, forced: [" +
              force +
              "], useSessionCookie: [" +
              useSessionCookie +
              "], seconds to expire: [" +
          ...
          setValueInStorage(
            "cly_session",
            getTimestamp() + sessionCookieTimeout * 60
          );
        }
      } else {
    ...
    this.end_session = function (sec, force) {
      log(
        logLevelEnums.INFO,
        "end_session, Ending the current session. There was an on going session:[" +
          sessionStarted +
          "]"
      );
      if (this.check_consent(featureEnums.SESSIONS)) {
    ...
    this.change_id = function (newId, merge) {
      ...
      if (!newId || typeof newId !== "string" || newId.length === 0) {
        log(
          logLevelEnums.ERROR,
          "change_id, The provided ID: [" + newId + "] is not a valid ID"
        );
        return;
      }
      if (offlineMode) {
      ...
      if (this.device_id != newId) {
        ...
        log(
          logLevelEnums.INFO,
          "change_id, Changing ID from:[" + oldId + "] to [" + newId + "]"
        );
        if (merge) {
    ...
    this.add_event = function (event) {
      ...
      switch (event.key) {
        ...
        case internalEventKeyEnums.ACTION:
          respectiveConsent =
            this.check_consent(featureEnums.CLICKS) ||
            this.check_consent(featureEnums.SCROLLS);
          break;
        default:
    ...
    function add_cly_events(event, eventIdOverride) {
      ...
      if (self.ignore_visitor) {
        log(
          logLevelEnums.ERROR,
          "Adding event failed. Possible bot or user opt out"
        );
        return;
      }
      if (!event.key) {
        log(
          logLevelEnums.ERROR,
          "Adding event failed. Event must have a key property"
        );
        return;
      }
      if (!event.count) {
      ...
      event.segmentation = truncateObject(
        event.segmentation,
        self.maxKeyLength,
        self.maxValueSize,
        self.maxSegmentationValues,
        "add_cly_event",
        log
      );
      ...
      log(
        logLevelEnums.INFO,
        "With event ID: [" + e.id + "], successfully adding the last event:",
        e
      );
    }
    ...
    this.start_event = function (key) {
      if (!key || typeof key !== "string") {
        log(
          logLevelEnums.WARNING,
          "start_event, you have to provide a valid string key instead of: [" +
            key +
            "]"
        );
        return;
      ...
      log(
        logLevelEnums.INFO,
        "start_event, Starting timed event with key: [" + key + "]"
      );
      ...
      if (timedEvents[key]) {
        log(
          logLevelEnums.WARNING,
          "start_event, Timed event with key: [" + key + "] already started"
        );
        return;
      }
    ...
    this.cancel_event = function (key) {
      if (!key || typeof key !== "string") {
        log(
          logLevelEnums.WARNING,
          "cancel_event, you have to provide a valid string key instead of: [" +
            key +
            "]"
        );
        return false;
      ...
      log(
        logLevelEnums.INFO,
        "cancel_event, Canceling timed event with key: [" + key + "]"
      );
      ...
      if (timedEvents[key]) {
        ...
        log(
          logLevelEnums.INFO,
          "cancel_event, Timed event with key: [" + key + "] is canceled"
        );
        return true;
      }
      log(
        logLevelEnums.WARNING,
        "cancel_event, Timed event with key: [" + key + "] was not found"
      );
      return false;
    };
    ...
    this.end_event = function (event) {
      if (!event) {
        log(
          logLevelEnums.WARNING,
          "end_event, you have to provide a valid string key or event object instead of: [" +
            event +
            "]"
        );
        return;
      ...
      if (!event.key) {
        log(
          logLevelEnums.ERROR,
          "end_event, Timed event must have a key property"
        );
        return;
      }
      if (!timedEvents[event.key]) {
        log(
          logLevelEnums.ERROR,
          "end_event, Timed event with key: [" + event.key + "] was not started"
        );
        return;
      }
    ...
    this.recordDirectAttribution = function (campaign_id, campaign_user_id) {
      ...
      if (this.check_consent(featureEnums.ATTRIBUTION)) {
        campaign_id =
          campaign_id || getValueFromStorage("cly_cmp_id") || "cly_organic";
        campaign_user_id =
          campaign_user_id || getValueFromStorage("cly_cmp_uid");
        if (campaign_user_id) {
          toRequestQueue({
    ...
    this.user_details = function (user) {
      log(
        logLevelEnums.INFO,
        "user_details, Trying to add user details: ",
        user
      );
      if (this.check_consent(featureEnums.USERS)) {
        ...
        user.name = truncateSingleValue(
          user.name,
          self.maxValueSize,
          "user_details",
          log
        );
        user.username = truncateSingleValue(
          user.username,
          self.maxValueSize,
          "user_details",
          log
        );
        user.email = truncateSingleValue(
          user.email,
          self.maxValueSize,
          "user_details",
          log
        );
        user.organization = truncateSingleValue(
          user.organization,
          self.maxValueSize,
          "user_details",
          log
        );
        user.phone = truncateSingleValue(
          user.phone,
          self.maxValueSize,
          "user_details",
          log
        );
        user.picture = truncateSingleValue(
          user.picture,
          4096,
          "user_details",
          log
        );
        user.gender = truncateSingleValue(
          user.gender,
          self.maxValueSize,
          "user_details",
          log
        );
        user.byear = truncateSingleValue(
          user.byear,
          self.maxValueSize,
          "user_details",
          log
        );
        user.custom = truncateObject(
          user.custom,
          self.maxKeyLength,
          self.maxValueSize,
          self.maxSegmentationValues,
        ...
        toRequestQueue({
          user_details: JSON.stringify(
            createNewObjectFromProperties(user, props)
          ),
        });
      }
    };
     ...
     ************************* */
    var customData = {};
    var change_custom_property = function change_custom_property(
      key,
      value,
      mod
    ) {
    ...
    this.userData = {
      ...
      set: function set(key, value) {
        log(
          logLevelEnums.INFO,
          "[userData] set, Setting user's custom property value: [" +
            value +
            "] under the key: [" +
            key +
            "]"
      ...
      unset: function unset(key) {
        log(
          logLevelEnums.INFO,
          "[userData] unset, Resetting user's custom property with key: [" +
            key +
            "] "
        );
        customData[key] = "";
      ...
      set_once: function set_once(key, value) {
        log(
          logLevelEnums.INFO,
          "[userData] set_once, Setting user's unique custom property value: [" +
            value +
            "] under the key: [" +
            key +
            "] "
      ...
      increment: function increment(key) {
        log(
          logLevelEnums.INFO,
          "[userData] increment, Increasing user's custom property value under the key: [" +
            key +
            "] by one"
        );
      ...
      increment_by: function increment_by(key, value) {
        log(
          logLevelEnums.INFO,
          "[userData] increment_by, Increasing user's custom property value under the key: [" +
            key +
            "] by: [" +
            value +
            "]"
      ...
      multiply: function multiply(key, value) {
        log(
          logLevelEnums.INFO,
          "[userData] multiply, Multiplying user's custom property value under the key: [" +
            key +
            "] by: [" +
            value +
            "]"
      ...
      max: function max(key, value) {
        log(
          logLevelEnums.INFO,
          "[userData] max, Saving user's maximum custom property value compared to the value: [" +
            value +
            "] under the key: [" +
            key +
            "]"
      ...
      min: function min(key, value) {
        log(
          logLevelEnums.INFO,
          "[userData] min, Saving user's minimum custom property value compared to the value: [" +
            value +
            "] under the key: [" +
            key +
            "]"
      ...
      push_unique: function push_unique(key, value) {
        log(
          logLevelEnums.INFO,
          "[userData] push_unique, Pushing a unique value: [" +
            value +
            "] under the key: [" +
            key +
            "] to user's custom property array"
      ...
      pull: function pull(key, value) {
        log(
          logLevelEnums.INFO,
          "[userData] pull, Removing the value: [" +
            value +
            "] under the key: [" +
            key +
      ...
      save: function save() {
        log(
          logLevelEnums.INFO,
          "[userData] save, Saving changes to user's custom property"
        );
        if (self.check_consent(featureEnums.USERS)) {
          sendEventsForced(); // flush events to event queue to prevent a drill issue
          log(logLevelEnums.INFO, "user_details, flushed the event queue");
    ...
    this.report_trace = function (trace) {
      ...
      if (this.check_consent(featureEnums.APM)) {
        ...
        for (var i = 0; i < props.length; i++) {
          if (
            props[i] !== "apm_attr" &&
            typeof trace[props[i]] === "undefined"
          ) {
            log(
              logLevelEnums.WARNING,
              "report_trace, APM trace don't have the property: " + props[i]
            );
            return;
          }
        }
        // truncate trace name and metrics wrt internal limits
        trace.name = truncateSingleValue(
          trace.name,
          self.maxKeyLength,
          "report_trace",
          log
        );
        trace.app_metrics = truncateObject(
          trace.app_metrics,
          self.maxKeyLength,
          self.maxValueSize,
          self.maxSegmentationValues,
        ...
        log(
          logLevelEnums.INFO,
          "report_trace, Successfully adding APM trace: ",
          e
        );
      }
    };
    ...
    this.track_errors = function (segments) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "track_errors, window object is not available. Not tracking errors."
        );
        return;
      }
    ...
    this.add_log = function (record) {
      log(
        logLevelEnums.INFO,
        "add_log, Adding a new log of breadcrumbs: [ " + record + " ]"
      );
     ...
     * @param {function=} callback - Callback to notify with first param error and second param remote config object
     * */
    function fetch_remote_config_explicit(
      keys,
      omit_keys,
      optIn,
      api,
      ...
      log(
        logLevelEnums.INFO,
        "fetch_remote_config_explicit, Fetching sequence initiated"
      );
      var request = {
        method: "rc",
        av: self.app_version,
      ...
      if (self.check_consent(featureEnums.REMOTE_CONFIG)) {
        ...
        if (providedCall) {
          providedCall(
            new Error("Remote config requires explicit consent"),
            remoteConfigs
          );
        }
      }
    ...
    this.enrollUserToAb = function (keys) {
      log(
        logLevelEnums.INFO,
        "enrollUserToAb, Providing AB test keys to opt in for"
      );
      ...
      makeNetworkRequest(
        ...
        function (err, params, responseText) {
          ...
          try {
            ...
            log(
              logLevelEnums.ERROR,
              "enrollUserToAb, Had an issue while parsing the response: " + ex
            );
          }
          // JSON array can pass
        },
    ...
    this.get_remote_config = function (key) {
      log(
        logLevelEnums.INFO,
        "get_remote_config, Getting remote config from storage"
      );
    ...
    this.track_sessions = function () {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "track_sessions, window object is not available. Not tracking sessions."
        );
        return;
      }
    ...
    this.track_pageview = function (page, ignoreList, viewSegments) {
      if (!isBrowser && !page) {
        log(
          logLevelEnums.WARNING,
          "track_pageview, window object is not available. Not tracking page views is page is not provided."
        );
        return;
      }
        ...
        for (var i = 0; i < ignoreList.length; i++) {
          try {
            ...
            if (reg.test(page)) {
              log(
                logLevelEnums.INFO,
                "track_pageview, Ignoring the page: " + page
              );
              return;
            }
          } catch (ex) {
      ...
      log(
        logLevelEnums.VERBOSE,
        "track_pageview, last view is assigned:[" +
          lastView +
          "], current view ID is:[" +
          currentViewId +
          "], previous view ID is:[" +
      ...
      // so we fallback to old logic of landing
        ...
        if (
          matches &&
          matches[11] &&
          matches[11] !== window.location.hostname
        ) {
          segments.start = 1;
        }
      }
      ...
      if (freshUTMTags && Object.keys(freshUTMTags).length) {
        log(
          logLevelEnums.INFO,
          "track_pageview, Adding fresh utm tags to segmentation:[" +
            JSON.stringify(freshUTMTags) +
            "]"
        );
      ...
      if (isBrowser && isReferrerUsable()) {
        log(
          logLevelEnums.INFO,
          "track_pageview, Adding referrer to segmentation:[" +
            document.referrer +
            "]"
        );
    ...
    this.track_clicks = function (parent) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "track_clicks, window object is not available. Not tracking clicks."
        );
        return;
      }
      ...
      if (parent) {
        log(
          logLevelEnums.INFO,
          "track_clicks, Tracking the specified children:[" + parent + "]"
        );
      }
    ...
    this.track_scrolls = function (parent) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "track_scrolls, window object is not available. Not tracking scrolls."
        );
        return;
      }
      ...
      if (parent) {
        log(
          logLevelEnums.INFO,
          "track_scrolls, Tracking the specified children"
        );
      }
    ...
    this.track_links = function (parent) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "track_links, window object is not available. Not tracking links."
        );
        return;
      }
    ...
    this.track_forms = function (parent, trackHidden) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "track_forms, window object is not available. Not tracking forms."
        );
        return;
      }
      log(
        ...
        if (typeof form.elements !== "undefined") {
          for (var i = 0; i < form.elements.length; i++) {
            ...
            if (
              input &&
              input.type !== "password" &&
              input.className.indexOf("cly_user_ignore") === -1
            ) {
              if (
                typeof segmentation["input:" + getInputName(input)] ===
                "undefined"
              ) {
              ...
              if (input.nodeName.toLowerCase() === "select") {
                if (typeof input.multiple !== "undefined") {
                  segmentation["input:" + getInputName(input)].push(
                    getMultiSelectValues(input)
                  );
                } else {
                  segmentation["input:" + getInputName(input)].push(
          ...
          for (var key in segmentation) {
            if (
              segmentation[key] &&
              typeof segmentation[key].join === "function"
            ) {
    ...
    this.collect_from_forms = function (parent, useCustom) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "collect_from_forms, window object is not available. Not collecting from forms."
        );
        return;
      }
      log(
      ...
      function processForm(event) {
          ...
          for (i = 0; i < form.elements.length; i++) {
            ...
            if (input && input.type !== "password") {
              ...
              if (input.className.indexOf("cly_user_ignore") === -1) {
                ...
                if (
                  input.className &&
                  input.className.indexOf("cly_user_") !== -1
                ) {
                ...
                // check for email
                  ...
                  (input.type && input.type.toLowerCase() === "email") ||
                  (input.name &&
                  ...
                  (input.id &&
                    labelData[input.id] &&
                    labelData[input.id].toLowerCase().indexOf("email") !==
                      -1) ||
                  /[^@\s]+@[^@\s]+\.[^@\s]+/.test(value)
                ) {
                  if (!userdata.email) {
                  ...
                  (input.id &&
                    labelData[input.id] &&
                    labelData[input.id].toLowerCase().indexOf("username") !==
                      -1)
                ) {
                  ...
                  (input.name &&
                    (input.name.toLowerCase().indexOf("tel") !== -1 ||
                      input.name.toLowerCase().indexOf("phone") !== -1 ||
                      input.name.toLowerCase().indexOf("number") !== -1)) ||
                  (input.id &&
                    (input.id.toLowerCase().indexOf("tel") !== -1 ||
                      input.id.toLowerCase().indexOf("phone") !== -1 ||
                      input.id.toLowerCase().indexOf("number") !== -1)) ||
                  (input.id &&
                    labelData[input.id] &&
                    (labelData[input.id].toLowerCase().indexOf("tel") !== -1 ||
                      labelData[input.id].toLowerCase().indexOf("phone") !==
                        -1 ||
                      labelData[input.id].toLowerCase().indexOf("number") !==
                        -1))
                ) {
                  if (!userdata.phone) {
                  ...
                  (input.name &&
                    (input.name.toLowerCase().indexOf("org") !== -1 ||
                      input.name.toLowerCase().indexOf("company") !== -1)) ||
                  (input.id &&
                    (input.id.toLowerCase().indexOf("org") !== -1 ||
                      input.id.toLowerCase().indexOf("company") !== -1)) ||
                  (input.id &&
                    labelData[input.id] &&
                    (labelData[input.id].toLowerCase().indexOf("org") !== -1 ||
                      labelData[input.id].toLowerCase().indexOf("company") !==
                        -1))
                ) {
                  ...
                  (input.name &&
                    input.name.toLowerCase().indexOf("name") !== -1) ||
                  (input.id && input.id.toLowerCase().indexOf("name") !== -1) ||
                  (input.id &&
                    labelData[input.id] &&
                    labelData[input.id].toLowerCase().indexOf("name") !== -1)
                ) {
                  if (!userdata.name) {
                    userdata.name = "";
                  }
        ...
        if (hasUserInfo) {
          log(
            logLevelEnums.INFO,
            "collect_from_forms, Gathered user data",
            userdata
          );
          if (useCustom) {
            self.user_details({
    ...
    this.collect_from_facebook = function (custom) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "collect_from_facebook, window object is not available. Not collecting from Facebook."
        );
        return;
      }
      if (typeof FB === "undefined" || !FB || !FB.api) {
        log(
          logLevelEnums.ERROR,
          "collect_from_facebook, Facebook SDK is not available"
        );
        return;
      }
      log(
      ...
      FB.api("/me", function (resp) {
        ...
        if (
          resp.work &&
          resp.work[0] &&
          resp.work[0].employer &&
          resp.work[0].employer.name
        ) {
     ...
     *   }
     * */
    this.reportFeedbackWidgetManually = function (
      ...
      log(
        ...
        "reportFeedbackWidgetManually, Providing information about user with, provided result of the widget with  ID: [ " +
          CountlyFeedbackWidget._id +
          " ] and type: [" +
          CountlyFeedbackWidget.type +
          "]"
      );
      ...
      if (widgetResult === null) {
        ...
        event.segmentation = addNewProperties(
          event.segmentation,
          widgetResult,
          props
        );
      ...
      log(
        logLevelEnums.INFO,
        "reportFeedbackWidgetManually, Reporting " + type + ": ",
        event
      );
      add_cly_events(event);
    };
    ...
    this.show_feedback_popup = function (id) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "show_feedback_popup, window object is not available. Not showing feedback popup."
        );
        return;
      }
      log(
    ...
    this.presentRatingWidgetWithID = function (id) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "presentRatingWidgetWithID, window object is not available. Not showing rating widget popup."
        );
        return;
      }
      log(
        ...
        makeNetworkRequest(
          ...
          function (err, params, responseText) {
            ...
            try {
              ...
              log(
                logLevelEnums.ERROR,
                "presentRatingWidgetWithID, JSON parse failed: " +
                  JSONParseError
              );
            }
            // JSON array can pass
    ...
    this.initializeRatingWidgets = function (enableWidgets) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "initializeRatingWidgets, window object is not available. Not initializing rating widgets."
        );
        return;
      }
      log(
      ...
      var stickers = document.getElementsByClassName(
      ...
      makeNetworkRequest(
        ...
        function (err, params, responseText) {
          ...
          try {
              ...
              if (widgets[i].is_active === "true") {
                ...
                if (target_devices[currentDevice]) {
                  ...
                  if (typeof widgets[i].hide_sticker === "string") {
                    widgets[i].hide_sticker =
                      widgets[i].hide_sticker === "true";
                  }
                  // is target_page option provided as "All"?
                  if (
                    widgets[i].target_page === "all" &&
                    !widgets[i].hide_sticker
                  ) {
                  ...
                  // is target_page option provided as "selected"?
                    ...
                    for (var k = 0; k < pages.length; k++) {
                      var isWildcardMatched =
                        pages[k].substr(0, pages[k].length - 1) ===
                        window.location.pathname.substr(0, pages[k].length - 1);
                      var isFullPathMatched =
                        pages[k] === window.location.pathname;
                      var isContainAsterisk = pages[k].includes("*");
                      if (
                        ((isContainAsterisk && isWildcardMatched) ||
                          isFullPathMatched) &&
                        !widgets[i].hide_sticker
                      ) {
                        processWidget(widgets[i], true);
            ...
            log(
              logLevelEnums.ERROR,
              "initializeRatingWidgets, JSON parse error: " + JSONParseError
            );
          }
          // JSON array can pass
        },
    ...
    this.enable_feedback = function (params) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "enable_feedback, window object is not available. Not enabling feedback."
        );
        return;
      }
      log(
    ...
    this.enableRatingWidgets = function (params) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "enableRatingWidgets, window object is not available. Not enabling rating widgets."
        );
        return;
      }
      log(
    ...
    this.getFeedbackWidgetData = function (CountlyFeedbackWidget, callback) {
      if (!CountlyFeedbackWidget.type) {
        log(
          ...
          "getFeedbackWidgetData, Expected the provided widget object to have a type but got: [" +
            JSON.stringify(CountlyFeedbackWidget) +
            "], aborting."
        );
        return;
      }
      ...
      if (CountlyFeedbackWidget.type === "nps") {
        ...
        log(
          logLevelEnums.ERROR,
          "getFeedbackWidgetData, Unknown type info: [" +
            CountlyFeedbackWidget.type +
            "]"
        );
        return;
     ...
     * */
    this.present_feedback_widget = function (
      presentableFeedback,
      id,
      className,
      feedbackWidgetSegmentation
    ) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "present_feedback_widget, window object is not available. Not presenting feedback widget."
        );
        return;
      }
      ...
      if (
        !presentableFeedback ||
        _typeof(presentableFeedback) !== "object" ||
      ...
      log(
        logLevelEnums.INFO,
        "present_feedback_widget, Adding segmentation to feedback widgets:[" +
          JSON.stringify(feedbackWidgetSegmentation) +
          "]"
      );
      if (
        !feedbackWidgetSegmentation ||
        _typeof(feedbackWidgetSegmentation) !== "object" ||
        Object.keys(feedbackWidgetSegmentation).length === 0
      ) {
        log(
          logLevelEnums.DEBUG,
          "present_feedback_widget, Segmentation is not an object or empty"
        );
      ...
      try {
        ...
        if (presentableFeedback.type === "nps") {
          log(
            logLevelEnums.DEBUG,
            "present_feedback_widget, Widget type: nps."
          );
          url += "/feedback/nps";
        } else if (presentableFeedback.type === "survey") {
          log(
            logLevelEnums.DEBUG,
            "present_feedback_widget, Widget type: survey."
          );
          url += "/feedback/survey";
        } else if (presentableFeedback.type === "rating") {
          log(
            logLevelEnums.DEBUG,
            "present_feedback_widget, Widget type: rating."
          );
          url += "/feedback/rating";
        } else {
          log(
        ...
        iframe.onload = function () {
          ...
          if (initiated) {
            ...
            document.getElementById(
              "countly-" +
                feedbackWidgetFamily +
                "-wrapper-" +
                presentableFeedback._id
            ).style.display = "none";
        ...
        while (overlay) {
          ...
          log(
            logLevelEnums.DEBUG,
            "present_feedback_widget, Removing past overlay."
          );
        }
        var wrapper = document.getElementsByClassName(
          "countly-" + feedbackWidgetFamily + "-wrapper"
        );
        for (var i = 0; i < wrapper.length; i++) {
          ...
          log(
            logLevelEnums.DEBUG,
            "present_feedback_widget, Removed a wrapper."
          );
        }
        ...
        if (presentableFeedback.type === "survey") {
          // Set popup position
          wrapper.className =
            wrapper.className + " " + presentableFeedback.appearance.position;
        }
        ...
        if (presentableFeedback.type === "rating") {
          ...
          ratingsOverlay.id =
            "countly-ratings-overlay-" + presentableFeedback._id;
          ...
          add_event_listener(
            ...
            function () {
              document.getElementById(
                "countly-ratings-wrapper-" + presentableFeedback._id
              ).style.display = "none";
            }
          );
        }
        ...
        log(
          logLevelEnums.DEBUG,
          "present_feedback_widget, Appended the iframe"
        );
        add_event_listener(window, "message", function (e) {
          ...
          try {
            ...
            log(
              logLevelEnums.DEBUG,
              "present_feedback_widget, Parsed response message " + data
            );
          } catch (ex) {
            log(
              logLevelEnums.ERROR,
              "present_feedback_widget, Error while parsing message body " + ex
            );
          }
          if (!data.close) {
            log(
      ...
      function showSurvey(feedback) {
        document.getElementById(
          "countly-surveys-wrapper-" + feedback._id
        ).style.display = "block";
        document.getElementById("csbg").style.display = "block";
      }
      ...
      function showRatingForFeedbackWidget(feedback) {
        ...
        if (!feedback.appearance.hideS) {
          ...
          sticker.style.color =
            feedback.appearance.text_color.length < 7
              ? "#" + feedback.appearance.text_color
              : feedback.appearance.text_color;
          sticker.style.backgroundColor =
            feedback.appearance.bg_color.length < 7
              ? "#" + feedback.appearance.bg_color
              : feedback.appearance.bg_color;
          sticker.className =
            "countly-feedback-sticker  " +
            feedback.appearance.position +
            "-" +
            feedback.appearance.size;
          sticker.id = "countly-feedback-sticker-" + feedback._id;
          document.body.appendChild(sticker);

          // sticker event handler
          add_event_listener(
            document.getElementById("countly-feedback-sticker-" + feedback._id),
            "click",
            function () {
              document.getElementById(
                "countly-ratings-wrapper-" + feedback._id
              ).style.display = "flex";
              document.getElementById("csbg").style.display = "block";
            }
          );
        ...
        add_event_listener(
          document.getElementById(
            "countly-feedback-close-icon-" + feedback._id
          ),
          "click",
          function () {
            document.getElementById(
      ...
      if (this.check_consent(featureEnums.CRASHES) && err) {
        ...
        if (
          ...
          if (splittedError.length > self.maxStackTraceLinesPerThread) {
            splittedError = splittedError.splice(
              0,
              self.maxStackTraceLinesPerThread
            );
          }
          ...
          for (var i = 0, len = splittedError.length; i < len; i++) {
            if (splittedError[i].length > self.maxStackTraceLineLength) {
              splittedError[i] = splittedError[i].substring(
                0,
                self.maxStackTraceLineLength
              );
            }
          }
        ...
        try {
          ...
          log(
            logLevelEnums.ERROR,
            "Could not get the experimental-webgl context: " + ex
          );
        }
    ...
    function checkIgnore() {
      log(
        logLevelEnums.INFO,
        "checkIgnore, Checking if user or visit should be ignored"
      );
      if (
        self.ignore_prefetch &&
        isBrowser &&
        ...
        log(
          logLevelEnums.DEBUG,
          "checkIgnore, Ignoring visit due to prerendering"
        );
      }
    ...
    function processWidget(currentWidget, hasSticker) {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "processWidget, window object is not available. Not processing widget."
        );
        return;
      }
      // prevent widget create process if widget exist with same id
      var isDuplicate = !!document.getElementById(
        "countly-feedback-sticker-" + currentWidget._id
      );
      if (isDuplicate) {
      ...
      try {
        ...
        add_event_listener(
          document.getElementById(
            "countly-feedback-close-icon-" + currentWidget._id
          ),
          "click",
          function () {
            document.getElementById(
              "countly-iframe-wrapper-" + currentWidget._id
            ).style.display = "none";
        ...
        if (hasSticker) {
          ...
          var sticker = document.createElement("div");
          sticker.style.color =
            currentWidget.trigger_font_color.length < 7
              ? "#" + currentWidget.trigger_font_color
              : currentWidget.trigger_font_color;
          sticker.style.backgroundColor =
            currentWidget.trigger_bg_color.length < 7
              ? "#" + currentWidget.trigger_bg_color
              : currentWidget.trigger_bg_color;
          sticker.className =
            "countly-feedback-sticker  " +
            currentWidget.trigger_position +
            "-" +
            currentWidget.trigger_size;
        ...
        log(
          logLevelEnums.ERROR,
          "Somethings went wrong while element injecting process: " + e
        );
      }
    }
    ...
    function toRequestQueue(request) {
      if (self.ignore_visitor) {
        log(
          logLevelEnums.WARNING,
          "User is opt_out will ignore the request: " + request
        );
        return;
      }
      ...
      if (sessionStarted && autoExtend && trackTime) {
        ...
        if (last - lastBeat > sessionUpdate) {
          ...
          if (self.hcErrorCount > 0) {
            setValueInStorage(
              healthCheckCounterEnum.errorCount,
              self.hcErrorCount
            );
          }
          if (self.hcWarningCount > 0) {
            setValueInStorage(
              healthCheckCounterEnum.warningCount,
              self.hcWarningCount
            );
          }
        }
    ...
    function isReferrerUsable(customReferrer) {
      ...
      if (typeof referrer === "undefined" || referrer.length === 0) {
        log(
          logLevelEnums.DEBUG,
          "Invalid referrer:[" + referrer + "], ignoring."
        );
      } else {
        ...
        if (!matches) {
          log(
            logLevelEnums.DEBUG,
            "Referrer is corrupt:[" + referrer + "], ignoring."
          );
        } else if (!matches[11]) {
          log(
            logLevelEnums.DEBUG,
            "No path found in referrer:[" + referrer + "], ignoring."
          );
        } else if (matches[11] === window.location.hostname) {
          log(
            logLevelEnums.DEBUG,
            "Referrer is current host:[" + referrer + "], ignoring."
          );
        } else {
          if (ignoreReferrers && ignoreReferrers.length) {
            ...
            for (var k = 0; k < ignoreReferrers.length; k++) {
              if (referrer.indexOf(ignoreReferrers[k]) >= 0) {
                log(
                  logLevelEnums.DEBUG,
                  "Referrer in ignore list:[" + referrer + "], ignoring."
                );
    ...
    function addCheckSum(paramsString) {
      var newParamsString = paramsString;
      if (self.salt) {
        var checksum = SHA256(newParamsString + self.salt);
        newParamsString += "&checksum256=" + checksum;
      }
    ...
    function sendXmlHttpRequest(
      ...
      try {
        ...
        var addCheckSumData = addCheckSum(data);
        if (method === "GET") {
          xhr.open("GET", url + "?" + addCheckSumData, true);
        } else {
        ...
        xhr.onreadystatechange = function () {
          if (this.readyState === 4) {
            ...
            if (useBroadResponseValidator) {
              ...
              isResponseValidated = isResponseValidBroad(
                this.status,
                this.responseText
              );
            } else {
              // only JSON object can pass
              isResponseValidated = isResponseValid(
                this.status,
                this.responseText
              );
            }
            if (isResponseValidated) {
              ...
              log(
                logLevelEnums.ERROR,
                functionName + " Invalid response from server"
              );
        ...
        log(
          logLevelEnums.ERROR,
          functionName +
            " Something went wrong while making an XML HTTP request: " +
            e
        );
        if (typeof callback === "function") {
    ...
    function sendFetchRequest(
      ...
      try {
        ...
        var body = null;
        var addCheckSumBody = null;
        params = params || {};
        if (self.force_post || prepareParams(params).length >= 2000) {
          method = "POST";
          body = prepareParams(params);
          addCheckSumBody = addCheckSum(body);
        } else {
          url += "?" + addCheckSum(prepareParams(params));
        }
        ...
        fetch(url, {
          ...
          .then(function (data) {
            log(
              logLevelEnums.DEBUG,
              functionName +
                " Fetch request completed wit status code: [" +
                response.status +
                "] and response: [" +
                data +
                "]"
          ...
          ["catch"](function (error) {
            log(
              logLevelEnums.ERROR,
              functionName + " Failed Fetch request: " + error
            );
            if (typeof callback === "function") {
              callback(true, params);
            }
        ...
        log(
          logLevelEnums.ERROR,
          functionName +
            " Something went wrong with the Fetch request attempt: " +
            e
        );
        if (typeof callback === "function") {
          callback(true, params);
    ...
    function isResponseValid(statusCode, str) {
      ...
      try {
        ...
        if (
          Object.prototype.toString.call(parsedResponse) !== "[object Object]"
        ) {
    ...
    function isResponseValidBroad(statusCode, str) {
      ...
      try {
        ...
        if (
          Object.prototype.toString.call(parsedResponse) !==
            "[object Object]" &&
          !Array.isArray(parsedResponse)
        ) {
          log(
            logLevelEnums.ERROR,
            "Http response is not JSON Object nor JSON Array"
          );
          return false;
        }
    ...
    function processScroll() {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "processScroll, window object is not available. Not processing scroll."
        );
        return;
      }
      scrollRegistryTopPosition = Math.max(
    ...
    function processScrollView() {
      if (!isBrowser) {
        log(
          logLevelEnums.WARNING,
          "processScrollView, window object is not available. Not processing scroll view."
        );
        return;
      }
    ...
    function getValueFromStorage(key, useLocalStorage, useRawKey) {
      ...
      if (
        self.storage === "none" ||
        (_typeof(self.storage) !== "object" && !isBrowser)
      ) {
        log(
          logLevelEnums.DEBUG,
          "Storage is disabled. Value with key: [" +
            key +
            "] won't be retrieved"
        );
        return;
      ...
      if (
        _typeof(self.storage) === "object" &&
        typeof self.storage.getItem === "function"
      ) {
    ...
    function setValueInStorage(key, value, useLocalStorage, useRawKey) {
      ...
      if (
        ...
        log(
          logLevelEnums.DEBUG,
          "Storage is disabled. Value with key: " + key + " won't be stored"
        );
        return;
      }
      ...
      if (typeof value !== "undefined" && value !== null) {
        ...
        if (
          _typeof(self.storage) === "object" &&
          typeof self.storage.setItem === "function"
        ) {
    ...
    function removeValueFromStorage(key, useLocalStorage, useRawKey) {
      ...
      if (
        ...
        log(
          logLevelEnums.DEBUG,
          "Storage is disabled. Value with key: " + key + " won't be removed"
        );
        return;
      }
      ...
      if (
        _typeof(self.storage) === "object" &&
        typeof self.storage.removeItem === "function"
      ) {
    ...
    function migrate() {
      if (getValueFromStorage(self.namespace + "cly_id", false, true)) {
        ...
        setValueInStorage(
          "cly_id",
          getValueFromStorage(self.namespace + "cly_id", false, true)
        );
        setValueInStorage(
          "cly_id_type",
          getValueFromStorage(self.namespace + "cly_id_type", false, true)
        );
        setValueInStorage(
          "cly_event",
          getValueFromStorage(self.namespace + "cly_event", false, true)
        );
        setValueInStorage(
          "cly_session",
          getValueFromStorage(self.namespace + "cly_session", false, true)
        );

        // filter out requests with correct app_key
        var requests = getValueFromStorage(
          self.namespace + "cly_queue",
          false,
          true
        );
        ...
        if (getValueFromStorage(self.namespace + "cly_cmp_id", false, true)) {
          setValueInStorage(
            "cly_cmp_id",
            getValueFromStorage(self.namespace + "cly_cmp_id", false, true)
          );
          setValueInStorage(
            "cly_cmp_uid",
            getValueFromStorage(self.namespace + "cly_cmp_uid", false, true)
          );
        }
        if (getValueFromStorage(self.namespace + "cly_ignore", false, true)) {
          setValueInStorage(
            "cly_ignore",
            getValueFromStorage(self.namespace + "cly_ignore", false, true)
          );
        }
    ...
    this.onStorageChange = function (key, newValue) {
      log(
        logLevelEnums.DEBUG,
        "onStorageChange, Applying storage changes for key:",
        key
      );
      log(
        logLevelEnums.DEBUG,
        "onStorageChange, Applying storage changes for value:",
        newValue
      );
      switch (key) {
        // queue of requests
    ...
    function saveRequestCounters(status, responseText) {
      ...
      setValueInStorage(
        healthCheckCounterEnum.errorMessage,
        self.hcErrorMessage
      );
    }
    /**
    ...
    function sendInstantHCRequest() {
      ...
      var curbedMessage = truncateSingleValue(
        self.hcErrorMessage,
        1000,
        "healthCheck",
        log
  ...
  Countly.deserialize = function (data) {
    ...
    try {
      ...
      if (checkIfLoggingIsOn()) {
        // eslint-disable-next-line no-console
        console.warn(
          "[WARNING] [Countly] deserialize, Could not parse the file:[" +
            data +
            "], error: " +
            e
        );
