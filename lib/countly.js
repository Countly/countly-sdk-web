    ...
    */
    ...
    var CountlyClass = function(ob) {
        ...
        * */
        ...
        /**
        * Interrupt timed event, which will skip timed event which was started before
        * @param {string} key - event name that will be used as key property
        * */
        this.interrupt_event = function(key) {
            log(logLevelEnums.INFO, "interrupt_event, Interrupting timed event with key: [" + key + "]");
            if (!timedEvents[key]) {
                log(logLevelEnums.WARNING, "interrupt_event, Timed event with key [" + key + "] not exist");
                return;
            }
            delete timedEvents[key];
            log(logLevelEnums.INFO, "interrupt_event, Interrupted timed event with key: [" + key + "]");
        };
