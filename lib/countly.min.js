...
j()-s[a.key],b.add_event(a),delete s[a.key],l("cly_timed",s)):f("Timed event with key "+key+" was not started"):f("Event must have key property")};b.user_details=function(a){f("Adding userdetails: ",a);n({user_details:JSON.stringify(Q(a,"name username email organization phone picture gender byear custom".split(" ")))})};b.reportConversion=function(a,d){a=a||l("cly_cmp_id");d=d||l("cly_cmp_uid");a&&d?n({campaign_id:a,campaign_user:d}):a&&n({campaign_id:a})};var p={},q=function(a,d,b){p[a]||(p[a]={});
"$push"==b||"$pull"==b||"$addToSet"==b?(p[a][b]||(p[a][b]=[]),p[a][b].push(d)):p[a][b]=d};b.userData={set:function(a,b){p[a]=b},set_once:function(a){q(a,1,"$setOnce")},increment:function(a){q(a,1,"$inc")},increment_by:function(a,b){q(a,b,"$inc")},multiply:function(a,b){q(a,b,"$mul")},max:function(a,b){q(a,b,"$max")},min:function(a,b){q(a,b,"$min")},push:function(a,b){q(a,b,"$push")},push_unique:function(a,b){q(a,b,"$addToSet")},pull:function(a,b){q(a,b,"$pull")},save:function(){n({user_details:JSON.stringify({custom:p})});
