...
d;if("undefined"!==typeof a.elements)for(var k=0;k<a.elements.length;k++)if(d=a.elements[k],l["input:"+c(d)]=[],"select"==d.nodeName.toLowerCase())if("undefined"!==typeof d.multiple){var m=[];if("undefined"!==typeof d.options)for(var p=0;p<d.options.length;p++)d.options[p].selected&&m.push(d.options[p].value);l["input:"+c(d)].push(m.join(","))}else l["input:"+c(d)].push(d.options[d.selectedIndex].value);else"input"==d.nodeName.toLowerCase()?"undefined"!==typeof d.type?"checkbox"==d.type.toLowerCase()||
"radio"==d.type.toLowerCase()?d.checked&&l["input:"+c(d)].push(d.value):l["input:"+c(d)].push(d.value):l["input:"+c(d)].push(d.value):"textarea"==d.nodeName.toLowerCase()?l["input:"+c(d)].push(d.value):"undefined"!==typeof d.value&&l["input:"+c(d)].push(d.value);for(var n in l)l[n]=l[n].join(",");b.add_event({key:"formSubmit",segmentation:l});b.end_session();Y(f);setTimeout(function(){a.submit()},1E3)}})}function e(){if("undefined"!==typeof a.getElementsByTagName)for(var c=a.getElementsByTagName("form"),
b=0;b<c.length;b++)f(c[b]);else k("Can't track forms")}a=a||document;"complete"===document.readyState?e():q(window,"load",e)};b.collect_from_forms=function(a,c){function f(e){var f=!1;q(e,"submit",function(){if(!f){f=!0;var h={},l=!1,d;if("undefined"!==typeof e.elements){var j={},m=a.getElementsByTagName("LABEL"),p;for(p=0;p<m.length;p++)m[p].htmlFor&&""!==m[p].htmlFor&&(j[m[p].htmlFor]=m[p].innerText||m[p].textContent||m[p].innerHTML);for(p=0;p<e.elements.length;p++)if(d=e.elements[p],-1==d.className.indexOf("cly_user_ignore")){var n=
"";if("select"==d.nodeName.toLowerCase())if("undefined"!==typeof d.multiple){n=[];if("undefined"!==typeof d.options)for(m=0;m<d.options.length;m++)d.options[m].selected&&n.push(d.options[m].value);n=n.join()}else n=d.options[d.selectedIndex].value;else"input"==d.nodeName.toLowerCase()?"undefined"!==typeof d.type?"checkbox"==d.type.toLowerCase()||"radio"==d.type.toLowerCase()?"undefined"!==typeof d.checked&&(n=d.value):n=d.value:n=d.value:"textarea"==d.nodeName.toLowerCase()?n=d.value:"undefined"!==
typeof d.value&&(n=d.value);if(d.className&&-1!=d.className.indexOf("cly_user_")){d=d.className.split(" ");for(m=0;m<d.length;m++)if(0===d[m].indexOf("cly_user_")){h[d[m].replace("cly_user_","")]=n;l=!0;break}}else if(d.type&&"email"==d.type.toLowerCase()||d.name&&-1!=d.name.toLowerCase().indexOf("email")||d.id&&-1!=d.id.toLowerCase().indexOf("email")||d.id&&j[d.id]&&-1!=j[d.id].toLowerCase().indexOf("email")||/[^@\s]+@[^@\s]+\.[^@\s]+/.test(n))h.email||(h.email=n),l=!0;else if(d.name&&-1!=d.name.toLowerCase().indexOf("username")||
d.id&&-1!=d.id.toLowerCase().indexOf("username")||d.id&&j[d.id]&&-1!=j[d.id].toLowerCase().indexOf("username"))h.username||(h.username=n),l=!0;else if(d.name&&(-1!=d.name.toLowerCase().indexOf("tel")||-1!=d.name.toLowerCase().indexOf("phone")||-1!=d.name.toLowerCase().indexOf("number"))||d.id&&(-1!=d.id.toLowerCase().indexOf("tel")||-1!=d.id.toLowerCase().indexOf("phone")||-1!=d.id.toLowerCase().indexOf("number"))||d.id&&j[d.id]&&(-1!=j[d.id].toLowerCase().indexOf("tel")||-1!=j[d.id].toLowerCase().indexOf("phone")||
-1!=j[d.id].toLowerCase().indexOf("number")))h.phone||(h.phone=n),l=!0;else if(d.name&&(-1!=d.name.toLowerCase().indexOf("org")||-1!=d.name.toLowerCase().indexOf("company"))||d.id&&(-1!=d.id.toLowerCase().indexOf("org")||-1!=d.id.toLowerCase().indexOf("company"))||d.id&&j[d.id]&&(-1!=j[d.id].toLowerCase().indexOf("org")||-1!=j[d.id].toLowerCase().indexOf("company")))h.organization||(h.organization=n),l=!0;else if(d.name&&-1!=d.name.toLowerCase().indexOf("name")||d.id&&-1!=d.id.toLowerCase().indexOf("name")||
d.id&&j[d.id]&&-1!=j[d.id].toLowerCase().indexOf("name"))h.name||(h.name=""),h.name+=n+" ",l=!0}}l&&(k("Gathered user data",h),c?b.user_details({custom:h}):b.user_details(h));b.end_session()}})}function e(){if("undefined"!==typeof a.getElementsByTagName)for(var c=a.getElementsByTagName("form"),b=0;b<c.length;b++)f(c[b]);else k("Can't track forms")}a=a||document;"complete"===document.readyState?e():q(window,"load",e)};b.collect_from_facebook=function(a){FB&&FB.api&&FB.api("/me",function(c){var f={};
c.name&&(f.name=c.name);c.email&&(f.email=c.email);"male"==c.gender?f.gender="M":"female"==c.gender&&(f.gender="F");if(c.birthday){var e=c.birthday.split("/").pop();e&&4==e.length&&(f.byear=e)}c.work&&(c.work[0]&&c.work[0].employer&&c.work[0].employer.name)&&(f.organization=c.work[0].employer.name);if(a){f.custom={};for(var i in a){for(var e=a[i].split("."),g=c,h=0;h<e.length&&!(g=g[e[h]],"undefined"===typeof g);h++);"undefined"!==typeof g&&(f.custom[i]=g)}}b.user_details(f)})};var q=function(a,c,
b){"undefined"!==typeof a.addEventListener?a.addEventListener(c,b,!1):a.attachEvent("on"+c,b)},Y=function(a){"undefined"!==typeof window.event?window.event.returnValue=!1:"undefined"!==typeof a.preventDefault?a.preventDefault():a.returnValue=!1},r=function(a,c,b){function e(a,b,c){var e=new Date;e.setTime(e.getTime()+864E5*c);c="; expires="+e.toGMTString();document.cookie=a+"="+b+c+"; path=/"}var b=b||!1,i=!1,g;if("undefined"!==typeof localStorage){i=!0;try{localStorage.setItem("testLocal",!0)}catch(h){i=
