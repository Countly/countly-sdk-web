(function(b){function v(a){if(a.key){a.count||(a.count=1);var c=N(a,["key","count","sum","dur","segmentation"]);c.timestamp=O();var b=new Date;c.hour=b.getHours();c.dow=b.getDay();r.push(c);h("cly_event",r);i("Adding event: ",a)}else i("Event must have key property")}function P(){if(A){var a={name:A,segment:wa};b.check_consent("views")&&v({key:"[CLY]_view",dur:n()-E,segmentation:a});A=null}}function p(a){if(!b.ignore_visitor)if(!b.app_key||!b.device_id)i("app_key or device_id is missing");else{a.app_key=
b.app_key;a.device_id=b.device_id;a.sdk_name=xa;a.sdk_version=ya;b.check_consent("location")?(b.country_code&&(a.country_code=b.country_code),b.city&&(a.city=b.city),null!==b.ip_address&&(a.ip_address=b.ip_address)):a.location="";a.timestamp=O();var c=new Date;a.hour=c.getHours();a.dow=c.getDay();s.length>Q&&s.shift();s.push(a);h("cly_queue",s,!0)}}function F(){if(b.ignore_visitor)R=!1;else{R=!0;var a=0;if("undefined"!==typeof b.onload&&0<b.onload.length){for(a=0;a<b.onload.length;a++)if("function"===
...
r=[]):(a=r.splice(0,G),p({events:JSON.stringify(a)})),h("cly_event",r));0<s.length&&(U&&n()>ca)&&(U=!1,a=s.shift(),da(w)&&(y&&(clearTimeout(y),y=null),a.consent=JSON.stringify(w),w={}),i("Processing request",a),ea(a,function(a,b){i("Request Finished",b,a);a&&(s.unshift(b),ca=n()+V);h("cly_queue",s,!0);U=!0}));setTimeout(F,W)}}function fa(){return h("cly_id")||ga()}function ga(){var a=(new Date).getTime();return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(b){var d=(a+16*Math.random())%
16|0;a=Math.floor(a/16);return("x"==b?d:d&3|8).toString(16)})}function X(){var a={};a._app_version=b.app_version;a._ua=navigator.userAgent;if(screen.width){var c=screen.width?parseInt(screen.width):0,d=screen.height?parseInt(screen.height):0;if(0!==c&&0!==d){if(navigator.platform&&/iPad|iPhone|iPod/.test(navigator.platform)&&window.devicePixelRatio)c=Math.round(c*window.devicePixelRatio),d=Math.round(d*window.devicePixelRatio);else if(90===Math.abs(window.orientation))var f=c,c=d,d=f;a._resolution=
...
"function"===typeof c&&c(!0,a))};"GET"==g?d.send():d.send(f)}catch(e){i("Failed XML HTTP request",e),"function"===typeof c&&c(!0,a)}}function ka(a){var b=[],d;for(d in a)b.push(d+"="+encodeURIComponent(a[d]));return b.join("&")}function ma(a){return"/"==a.substr(a.length-1)?a.substr(0,a.length-1):a}function N(a,b){for(var d={},f,g=0;g<b.length;g++)f=b[g],"undefined"!==typeof a[f]&&(d[f]=a[f]);return d}function Y(a){"undefined"==typeof a.pageY&&("number"==typeof a.clientX&&document.documentElement)&&
...
a.appendChild(b);a=document.createElement("div");a.setAttribute("id","cly-loader");document.body.appendChild(a)}a.style.display="block"}function ra(){var a=document.getElementById("cly-loader");a&&(a.style.display="none")}var ya="18.01",xa="javascript_native_web",sa=!1,B=!1,la="/i",W=500,Q=1E3,s=[],r=[],J=[],z={},C=[],ia=null,S=!0,x,ta=0,A=null,E=0,aa=0,ca=0,V=60,K=20,L=0,T=60,G=10,H=0,U=!0,wa,R=!1,w={},q={},ha=/^(((([^:\/#\?]+:)?(?:(\/\/)((?:(([^:@\/#\?]+)(?:\:([^:@\/#\?]+))?)@)?(([^:\/#\?\]\[]+|\[[^\/\]@#?]+\])(?:\:([0-9]+))?))?)?)?((\/?(?:[^\/\?#]+\/+)*)([^\?#]*)))?(\?[^#]+)?)(#.*)?/,
